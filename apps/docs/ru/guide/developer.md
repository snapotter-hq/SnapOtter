...
Python 3.11+ нужен только если вы работаете над AI/ML-сайдкаром (удаление фона, апскейл, OCR).
...
|----------|--------------------------|------------------------------------|
| Фронтенд | http://localhost:1351     | Dev-сервер Vite, проксирует /api      |
| Бэкенд  | http://localhost:13490    | Fastify API (доступ через прокси)   |

Откройте http://localhost:1351 в браузере. Войдите под `admin` / `admin`. При первом входе вам будет предложено сменить пароль.
...
## Домены релизной версии {#release-version-domains}

SnapOtter намеренно имеет три домена версий. Не копируйте один домен в другой во время релиза:

- Версия выпуска приложения охватывает корневой манифест, все пакеты частной рабочей области и `APP_VERSION`. Semantic-release предоставляет это значение, а `pnpm version:sync <version>` обновляет каждую рабочую область перед выпуском приложения.
- OpenAPI `info.version` — стабильный публичный основной контракт API. Все локализованные спецификации остаются в `<major>.0.0` для совместимых выпусков приложений и изменяются только при переходе контракта API на новую основную версию.
- `docker/feature-manifest.json` сохраняет `imageVersion: 2.0.0` как неизменяемую устаревшую эпоху хранения пакетов функций. Эти пути к архивам версии 2 не являются версиями пакетов приложений. Accurate OCR использует формат времени выполнения v3 и записывает происхождение версии приложения отдельно.

`tests/unit/infra/release-version-policy.test.ts` обеспечивает соблюдение этих границ. Домен новой версии или миграция должны одновременно обновить этот контракт и соответствующий проект миграции артефакта.
Независимые значения API и устаревшего пакета находятся в `config/release-version-policy.json`; синхронизация версий приложения никогда не должна неявно перезаписывать этот файл политики.
