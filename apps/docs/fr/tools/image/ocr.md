...
**Traitement :** Renvoie `200` avec JSON lorsque OCR se termine à l'intérieur de la fenêtre synchrone. Les tâches plus longues renvoient `202` ; suivez le flux de progression SSE de la tâche jusqu'à son événement de terminal, dont `result` contient les mêmes champs OCR.
...
Si un champ de formulaire `clientJobId` est fourni, les événements de progression sont diffusés en continu. Une réponse `202` signifie que le client doit garder le flux ouvert jusqu'à l'événement `complete` ou `failed` du terminal :
...
- La réponse signale à la fois `requestedQuality` et `actualQuality`, ainsi que les versions du moteur, de l'appareil, du fournisseur, de l'exécution et du modèle, ainsi que tous les avertissements.
