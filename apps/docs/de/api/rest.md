...
Interaktive API-Dokumentation mit Beispielen für Anfragen und Antworten ist unter [http://localhost:1349/api/docs](http://localhost:1349/api/docs) verfügbar.
...
Schlüssel tragen das Präfix `si_` und werden als scrypt-Hashes gespeichert. Der Rohschlüssel wird einmalig angezeigt und ist danach nie wieder abrufbar.

### Authentifizierungs-Endpunkte {#auth-endpoints}

| Methode | Pfad | Zugriff | Beschreibung |
|--------|------|--------|-------------|
| `POST` | `/api/auth/login` | Öffentlich | Anmeldung, Sitzungstoken abrufen |
...
| `POST` | `/api/auth/register` | Admin | Einen neuen Benutzer erstellen |
...
| `POST` | `/api/auth/mfa/complete` | Öffentlich | Eine ausstehende MFA-Anmelde-Challenge abschließen |
...
| `GET` | `/api/auth/oidc/login` | Öffentlich | OIDC-Anmeldung starten, wenn OIDC aktiviert ist |
...
| `GET` | `/api/auth/saml/login` | Öffentlich | SAML-Anmeldung starten |
| `POST` | `/api/auth/saml/callback` | Öffentlich | SAML Assertion Consumer Service |

Wenn MFA für einen Benutzer aktiviert ist, gibt `POST /api/auth/login` statt eines Sitzungstokens `{"requiresMfa":true,"mfaToken":"...","mfaRequired":true|false}` zurück. Senden Sie diesen `mfaToken` zusammen mit einem TOTP- oder Wiederherstellungscode an `/api/auth/mfa/complete`.
...
| Methode | Pfad | Zugriff | Beschreibung |
|--------|------|--------|-------------|
| `GET` | `/api/v1/health` | Öffentlich | Einfacher Health-Check. Gibt `{"status":"healthy","version":"..."}` mit 200 zurück oder `{"status":"unhealthy"}` mit 503, wenn die Datenbank nicht erreichbar ist. |
| `GET` | `/api/v1/readyz` | Öffentlich | Readiness-Probe. Prüft PostgreSQL, Redis, Speicherplatz und, sofern konfiguriert, S3. Gibt 503 zurück, wenn die Instanz keinen Traffic empfangen soll. |
| `GET` | `/api/v1/admin/health` | Admin (`system:health`) | Detaillierte Diagnose einschließlich Betriebszeit, Speichermodus, Datenbankstatus, Queue-Zustand und GPU-Verfügbarkeit. |
...
`<section>` ist eines von `image`, `video`, `audio`, `pdf` oder `files`.
...
- `fileId` ist ein optionales Formularfeld, das auf ein bestehendes Element der Dateibibliothek verweist. Ist es vorhanden, wird die verarbeitete Ausgabe als neue Version gespeichert und die Antwort enthält `savedFileId`.
...
- **Jedes eingereihte Tool** kann 202 JSON zurückgeben, wenn es lange läuft oder das synchrone Wartefenster überschreitet: `{"jobId":"...","async":true}`. Verbinden Sie sich für den Fortschritt mit SSE und laden Sie nach Abschluss herunter (siehe [Fortschrittsverfolgung](#progress-tracking)).
- **Stapel-Routen** liefern für Tools, die im generischen Stapelregister registriert sind, ein direkt gestreamtes ZIP-Archiv (mit `X-Job-Id`-Header).
...
Jedes Preset legt das Ausgabeformat fest und delegiert an ein Basis-Tool wie `convert`, `convert-video`, `extract-audio`, `convert-audio`, `image-to-pdf`, `pdf-to-image`, `svg-to-raster` oder `convert-spreadsheet`. Die vollständige Routentabelle und optionale Einstellungen finden Sie unter [Konvertierungs-Presets](/de/tools/conversion-presets).

### Grundlagen {#essentials}

| Tool-ID | Name | Wichtige Einstellungen |
|---------|------|-------------|
| `resize` | Größe ändern | `width`, `height`, `fit` (cover/contain/fill/inside/outside), `percentage`, `withoutEnlargement` sowie 23 Social-Media-Presets |
...
| `favicon` | Favicon-Generator | `padding`, `backgroundColor`, `borderRadius` - erzeugt alle Standardgrößen |
...
| `pixelate` | Verpixeln | `blockSize` (2-128), `region` ({left, top, width, height} für teilweises Verpixeln) |
| `vignette` | Vignette | `strength` (0.1-1), `color` (hex), `radius`, `softness`, `roundness`, `centerX`, `centerY` |

### KI-Tools {#ai-tools}

Alle KI-Tools laufen auf Ihrer Hardware: standardmäßig auf der CPU oder über NVIDIA CUDA, wenn eine unterstützte NVIDIA-GPU verfügbar ist. Intel-/AMD-iGPU-Beschleunigung über VA-API, Quick Sync oder OpenCL wird für KI-Inferenz derzeit nicht unterstützt. Kein Internet erforderlich.
...
| `upscale` | Bild hochskalieren | RealESRGAN | `scale` (2/4), `model`, `faceEnhance`, `denoise`, `format`, `quality` |
| `erase-object` | Objekt-Radierer | LaMa (ONNX) | Maske wird als zweiter Dateiteil gesendet (Feldname `mask`), `format`, `quality` |
...
| `smart-crop` | Intelligentes Zuschneiden | MediaPipe + Sharp | `mode` (subject/face/trim), `strategy` (attention/entropy), `width`, `height`, `padding`, `facePreset` (closeup/head-shoulders/upper-body/half-body), `sensitivity`, `threshold`, `padToSquare`, `padColor`, `targetSize`, `quality` |
...
| `noise-removal` | Rauschentfernung | Gestuftes Entrauschen | `tier` (quick/balanced/quality/maximum), `strength`, `detailPreservation`, `colorNoise`, `format`, `quality` |
| `red-eye-removal` | Rote-Augen-Entfernung | Gesichtsmarker + Farbanalyse | `sensitivity`, `strength` |
...
| `passport-photo` | Passfoto | MediaPipe-Landmarken | Zweiphasiger Ablauf. Analyse verwendet Multipart `file`; die Generierung verwendet JSON mit `countryCode`, `bgColor`, `printLayout` (none/4x6/a4), Landmarken, Bildabmessungen |
| `content-aware-resize` | Inhaltsbewusste Größenänderung | Seam Carving (caire) | `width`, `height`, `protectFaces`, `blurRadius`, `sobelThreshold`, `square` |
| `transparency-fixer` | PNG-Transparenz-Reparatur | BiRefNet HR-Matting | `defringe` (0-100), `outputFormat` (png/webp) |
...
| Tool-ID | Name | Wichtige Einstellungen |
|---------|------|-------------|
| `collage` | Collage / Raster | `template` (25+ Layouts), `gap`, `backgroundColor`, `borderRadius` - mehrere Dateien |
| `stitch` | Zusammenfügen / Kombinieren | `direction` (horizontal/vertical/grid), `gap`, `backgroundColor`, `alignment` - mehrere Dateien |
| `split` | Bild teilen | `mode` (grid/rows/cols), `rows`, `cols`, `tileWidth`, `tileHeight` |
| `border` | Rahmen & Rand | `width`, `color`, `style` (solid/gradient/pattern), `borderRadius`, `padding`, `shadow` |
...
| `circle-crop` | Kreiszuschnitt | `zoom` (1-5), `offsetX`, `offsetY`, `borderWidth`, `borderColor`, `background` (transparent/hex), `outputSize` |
...
| `sprite-sheet` | Sprite-Sheet | `columns` (1-16), `padding`, `background` (hex), `format` (png/webp/jpeg), `quality` - mehrere Dateien (2-64 Bilder) |

### Format & Konvertierung {#format-conversion}

| Tool-ID | Name | Wichtige Einstellungen |
|---------|------|-------------|
| `svg-to-raster` | SVG zu Rastergrafik | `format` (png/jpeg/webp/avif/tiff/gif/heif), `width`, `height`, `scale`, `dpi`, `background` |
...
| `trim-video` | Video kürzen | `startS`, `endS`, `precise` (bool, framegenauer Schnitt) |
| `mute-video` | Video stummschalten | - |
| `video-to-gif` | Video zu GIF | `fps` (1-30), `width`, `startS`, `durationS` (max. 60s) |
| `resize-video` | Videogröße ändern | `width`, `height`, `preset` (custom/2160p/1440p/1080p/720p/480p/360p) |
| `crop-video` | Video zuschneiden | `width`, `height`, `x`, `y` |
| `rotate-video` | Video drehen | `transform` (cw90/ccw90/180/hflip/vflip) |
| `change-fps` | FPS ändern | `fps` (1-120) |
| `video-color` | Videofarbe | `brightness`, `contrast`, `saturation`, `gamma` |
| `video-speed` | Videogeschwindigkeit | `factor` (0.25-4), `keepPitch` (bool) |
| `reverse-video` | Video umkehren | - (max. 5 Minuten) |
| `video-loudnorm` | Audio normalisieren | - (EBU R128) |
| `aspect-pad` | Seitenverhältnis-Auffüllung | `target` (16:9/9:16/1:1/4:3/3:4), `color` (hex) |
...
| `merge-videos` | Videos zusammenführen | - (mehrere Dateien, auf die Auflösung des ersten Videos normalisiert) |
...
| `images-to-video` | Bilder zu Video | `secondsPerImage` (0.5-10), `resolution` (1080p/720p/square), `fps` - mehrere Dateien |
...
| `trim-audio` | Audio kürzen | `startS`, `endS` |
...
| `audio-speed` | Audiogeschwindigkeit | `factor` (0.25-4) |
| `pitch-shift` | Tonhöhenverschiebung | `semitones` (-12 bis 12) |
| `audio-channels` | Audiokanäle | `mode` (stereo-to-mono/mono-to-stereo/swap) |
...
| `merge-audio` | Audio zusammenführen | `format` (mp3/wav/flac/m4a) - mehrere Dateien |
| `split-audio` | Audio teilen | `mode` (time/parts/silence), `segmentS`, `parts`, `thresholdDb`, `minSilenceS` |
...
| Tool-ID | Name | Wichtige Einstellungen |
|---------|------|-------------|
| `merge-pdf` | PDFs zusammenführen | - (mehrere Dateien, bis zu 20 PDFs) |
| `split-pdf` | PDF teilen | `mode` (range/every), `range`, `everyN` (1-500) |
...
| `linearize-pdf` | PDF für Web optimieren | - (Linearisierung für schnelle Web-Anzeige) |
| `grayscale-pdf` | PDF in Graustufen | - |
| `pdfa-convert` | PDF/A-Konvertierung | - (Archiv-PDF/A-2) |
...
| `flatten-pdf` | PDF abflachen | - (bäckt Formulare und Anmerkungen ein) |
| `redact-pdf` | PDF schwärzen | `terms` (string[]), `caseSensitive` (bool) |
...
| `convert-spreadsheet` | Tabelle konvertieren | `format` (xlsx/ods/csv) |
...
| `split-csv` | CSV teilen | `rowsPerFile` (1-1000000), `keepHeader` (bool) |
| `merge-csvs` | CSVs zusammenführen | - (mehrere Dateien, übereinstimmende Spalten) |
...
| `create-zip` | ZIP erstellen | - (mehrere Dateien, 2-50 Dateien) |
| `extract-zip` | ZIP entpacken | - (bombengeschützt) |

### HTML zu Bild {#html-to-image}

Erfassen Sie eine Webseite als Bild. Anders als andere Tools akzeptiert dieser Endpunkt `application/json` statt Multipart-Formulardaten (kein Datei-Upload nötig).
...
Einige Tools stellen zusätzliche Endpunkte über die Standardroute `POST /api/v1/tools/<section>/<toolId>` hinaus bereit:

| Methode | Pfad | Beschreibung |
|--------|------|-------------|
| `GET` | `/api/v1/tools/popular` | Gibt beliebte Tool-IDs zurück und greift auf eine kuratierte Standardliste zurück, wenn Nutzungsdaten spärlich sind |
...
| `POST` | `/api/v1/tools/image/passport-photo/analyze` | Phase 1: KI-Gesichtserkennung + Hintergrundentfernung. Gibt Gesichtsmarker und zwischengespeicherte Daten zurück. |
| `POST` | `/api/v1/tools/image/passport-photo/generate` | Phase 2: Zuschneiden, Größe ändern und Kacheln mithilfe der zwischengespeicherten Analyse. Keine erneute KI-Ausführung. |
| `POST` | `/api/v1/tools/image/gif-tools/info` | GIF-Metadaten abrufen (Frame-Anzahl, Abmessungen, Dauer) |
| `POST` | `/api/v1/tools/pdf/pdf-to-image/info` | PDF-Metadaten abrufen (Seitenzahl, Abmessungen) |
...
| `POST` | `/api/v1/tools/image/svg-to-raster/batch` | Mehrere SVGs im Stapel zu Rastergrafik konvertieren |
| `POST` | `/api/v1/tools/image/image-enhancement/analyze` | Bildqualität analysieren und Verbesserungsempfehlungen zurückgeben |
| `POST` | `/api/v1/tools/image/optimize-for-web/preview` | Leichtgewichtige Vorschau für das Live-Feintuning von Parametern. Gibt ein optimiertes Bild mit Größen-Headern zurück. |

## Stapelverarbeitung {#batch-processing}

Wenden Sie ein generisches, stapelfähiges Tool auf mehrere Dateien gleichzeitig an. Gibt ein ZIP-Archiv zurück. Benutzerdefinierte Mehrdatei- oder mehrstufige Routen, etwa PDF-Signierung, PDF-OCR und PDF-zu-Bild-Preset-Routen, verwenden ihren eigenen Endpunktvertrag statt der generischen `/batch`-Route.
...
Die Nebenläufigkeit wird über `CONCURRENT_JOBS` gesteuert (Standard: automatisch aus den CPU-Kernen erkannt). `MAX_BATCH_SIZE` begrenzt die Anzahl der Dateien pro Stapel (Standard: 100; 0 für unbegrenzt setzen).
...
| `DELETE` | `/api/v1/pipeline/:id` | Löschen (Besitzer oder Admin) |
| `GET` | `/api/v1/pipeline/tools` | Für Pipeline-Schritte gültige Tool-IDs auflisten |

## Fortschrittsverfolgung {#progress-tracking}

Lang laufende Jobs, eingereihte Tools, Stapeljobs und Pipelines melden den Fortschritt in Echtzeit über Server-Sent Events. Der Fortschritts-Stream ist öffentlich und über die Job-ID adressiert, sodass Clients keinen Authorization-Header senden müssen, um ihn zu lesen.
...
Persistente Dateispeicherung mit Versionsverlauf.

| Methode | Pfad | Beschreibung |
|--------|------|-------------|
| `POST` | `/api/v1/upload` | Dateien in den Arbeitsbereich hochladen (temporäre Verarbeitung) |
| `POST` | `/api/v1/files/upload` | Dateien in die persistente Dateibibliothek hochladen |
...
| `DELETE` | `/api/v1/files` | Dateien und ihre Versionsketten im Stapel löschen (Body: `{ ids: [...] }`) |
...
| `POST` | `/api/v1/preview/generate` | Eine On-Demand-MP4- oder -MP3-Vorschau für eine hochgeladene Mediendatei erzeugen, ohne sie zuvor zu speichern |
| `GET` | `/api/v1/download/:jobId/:filename` | Eine verarbeitete Datei aus einem Arbeitsbereich herunterladen |

Um ein Tool-Ergebnis automatisch in der Bibliothek zu speichern, fügen Sie `fileId` als Multipart-Formularfeld hinzu, das auf eine bestehende Bibliotheksdatei verweist. Das verarbeitete Ergebnis wird als neue Version gespeichert.

## API-Schlüssel-Verwaltung {#api-key-management}

| Methode | Pfad | Zugriff | Beschreibung |
|--------|------|--------|-------------|
| `POST` | `/api/v1/api-keys` | Auth | Neuen Schlüssel generieren - einmalig angezeigt |
| `GET` | `/api/v1/api-keys` | Auth | Schlüssel auflisten (Name, ID, lastUsedAt - nicht den Rohschlüssel) |
...
| `DELETE` | `/api/v1/teams/:id` | Admin (`teams:manage`) | Team löschen (Standardteam und Teams mit Mitgliedern können nicht gelöscht werden) |

## Einstellungen {#settings}

Schlüssel-Wert-Konfiguration zur Laufzeit (von jedem authentifizierten Benutzer lesbar, nur von Admins schreibbar).

| Methode | Pfad | Beschreibung |
|--------|------|-------------|
| `GET` | `/api/v1/settings` | Alle Einstellungen abrufen |
| `PUT` | `/api/v1/settings` | Einstellungen im Stapel aktualisieren (JSON-Body mit Schlüssel-Wert-Paaren) |
| `GET` | `/api/v1/settings/:key` | Eine bestimmte Einstellung per Schlüssel abrufen |

Bekannte Schlüssel: `disabledTools` (JSON-Array von Tool-IDs), `enableExperimentalTools` (bool-String), `loginAttemptLimit` (Zahl).

## Einstellungen (Präferenzen) {#preferences}

Benutzerspezifische Präferenzen sind von den Instanzeinstellungen getrennt. Jeder authentifizierte Benutzer kann seine eigene Präferenzabbildung lesen und aktualisieren.

| Methode | Pfad | Beschreibung |
|--------|------|-------------|
| `GET` | `/api/v1/preferences` | Die Präferenzen des aktuellen Benutzers als `{ "preferences": { ... } }` abrufen |
| `PUT` | `/api/v1/preferences` | Einen oder mehrere Präferenzschlüssel für den aktuellen Benutzer einfügen/aktualisieren |

## Rollen {#roles}

Verwaltung benutzerdefinierter Rollen mit granularen Berechtigungen.

| Methode | Pfad | Zugriff | Beschreibung |
|--------|------|--------|-------------|
| `GET` | `/api/v1/roles` | Admin (`audit:read`) | Alle Rollen mit Benutzerzahlen auflisten |
...
| `DELETE` | `/api/v1/roles/:id` | Admin (`security:manage`) | Eine benutzerdefinierte Rolle löschen (integrierte Rollen können nicht gelöscht werden; betroffene Benutzer wechseln zur Rolle `user` zurück) |
...
Nur für Admins zugänglicher Endpunkt zur Überprüfung sicherheitsrelevanter Aktionen.
...
| Methode | Pfad | Zugriff | Beschreibung |
|--------|------|--------|-------------|
| `GET` | `/api/v1/config/analytics` | Öffentlich | Die effektive Analytics-Konfiguration abrufen (PostHog-Schlüssel, Sentry-DSN, Sample-Rate). Schlüssel, DSN und Instanz-ID sind leer, wenn Analytics deaktiviert ist, entweder durch den Compile-Time-Bake oder die Instanzeinstellung `analyticsEnabled`. |
| `POST` | `/api/v1/feedback` | Auth | Explizites Benutzer-Feedback an das konfigurierte PostHog-Projekt als `feedback_submitted` übermitteln. Die Route beachtet das Analytics-Gate, begrenzt Übermittlungen per Rate-Limit, entfernt Kontaktfelder, sofern `contactOk` nicht true ist, und akzeptiert niemals Dateiinhalte, Dateinamen, Upload-Pfade oder privaten Rohfehlertext. Ist Analytics deaktiviert, gibt sie `{ "ok": true, "accepted": false }` zurück. |
| `PUT` | `/api/v1/settings` | Admin (`settings:write`) | Das instanzweite Opt-out setzen. Senden Sie einen JSON-Body `{ "analyticsEnabled": "false" }`, um Analytics für alle zu deaktivieren, oder `"true"`, um es wieder zu aktivieren. |

## Features / KI-Bundles {#features-ai-bundles}

Verwalten Sie KI-Feature-Bundles (Installieren/Deinstallieren von KI-Modellpaketen in der Docker-Umgebung).
...
| `GET` | `/api/v1/admin/features/disk-usage` | Admin (`features:manage`) | Gesamten Speicherplatzverbrauch der KI-Modelle abrufen |
...
Betriebs-Endpunkte für Observability, Support, Nutzungsberichte und Backup-Status.
...
| `GET` | `/api/v1/enterprise/ip-allowlist` | Admin (`security:manage`) | Konfigurierte CIDR-Zulassungsliste lesen |
| `PUT` | `/api/v1/enterprise/ip-allowlist` | Admin (`security:manage`) | CIDR-Zulassungsliste mit Schutz vor Selbstausschluss aktualisieren |
...
| `POST` | `/api/v1/enterprise/scim/token` | Admin (`users:manage`) | Ein SCIM-Bearer-Token generieren, einmalig zurückgegeben |
...
| `POST` | `/api/v1/enterprise/webhooks/:index/test` | Admin (`webhooks:manage`) | Eine Test-Webhook-Payload senden |
...
| `GET` | `/api/v1/admin/migrations/pending` | Admin (`system:health`) | Paketierte Migrationen mit angewendeten Migrationen vergleichen |
| `GET` | `/api/v1/admin/upgrade-check` | Admin (`system:health`) | Upgrade-Bereitschaftsprüfungen ausführen |

### SCIM 2.0 {#scim-2-0}

SCIM-Discovery-Endpunkte sind öffentlich. Benutzer- und Gruppen-Endpunkte erfordern das oben generierte SCIM-Bearer-Token.
...
| `DELETE` | `/api/v1/scim/v2/Users/:id` | SCIM-Token | Einen Benutzer sanft deaktivieren |
...
| Methode | Pfad | Zugriff | Beschreibung |
|--------|------|--------|-------------|
| `GET` | `/api/v1/meme-templates` | Auth | Alle verfügbaren Meme-Vorlagen mit Textfeldpositionen auflisten |
| `GET` | `/api/v1/meme-templates/full/:filename` | Auth | Vorlagenbild in voller Größe bereitstellen |
| `GET` | `/api/v1/meme-templates/thumbs/:filename` | Auth | Vorlagen-Thumbnail bereitstellen |
| `GET` | `/api/v1/meme-templates/fonts/:filename` | Auth | Schriftdatei bereitstellen, die für das Rendern von Meme-Text verwendet wird |
...
| 422 | Verarbeitung nach Validierung fehlgeschlagen |
| 429 | Rate-Limit erreicht (siehe `RATE_LIMIT_PER_MIN`) |
