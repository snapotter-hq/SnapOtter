---
description: "Riferimento completo dell'API REST. Endpoint degli strumenti, elaborazione in batch, pipeline, libreria dei file, autenticazione, team e operazioni di amministrazione."
i18n_source_hash: eb73a14533a1
i18n_provenance: machine
i18n_output_hash: 6b605c74e25e
---
...
|--------|------|--------|-------------|
| `POST` | `/api/auth/login` | Pubblico | Accesso, ottieni un token di sessione |
...
| `PUT` | `/api/auth/users/:id` | Admin | Aggiorna il ruolo o il team di un utente |
| `POST` | `/api/auth/users/:id/reset-password` | Admin | Reimposta la password di un utente |
...
| `POST` | `/api/auth/mfa/enroll` | Auth | Avvia la registrazione TOTP per la MFA. Richiede la funzionalità enterprise `mfa` |
| `POST` | `/api/auth/mfa/verify` | Auth | Conferma la registrazione MFA con un codice TOTP |
| `POST` | `/api/auth/mfa/complete` | Pubblico | Completa una richiesta di accesso MFA in sospeso |
| `POST` | `/api/auth/mfa/disable` | Auth | Disabilita la MFA per l'utente corrente |
| `POST` | `/api/auth/users/:id/mfa/reset` | Admin (`users:manage`) | Reimposta la MFA per un utente |
...
| `GET` | `/api/auth/saml/metadata` | Pubblico | XML dei metadati del SP SAML quando SAML è abilitato |
| `GET` | `/api/auth/saml/login` | Pubblico | Avvia l'accesso SAML |
| `POST` | `/api/auth/saml/callback` | Pubblico | Servizio consumatore di asserzioni SAML |

Quando la MFA è abilitata per un utente, `POST /api/auth/login` restituisce `{"requiresMfa":true,"mfaToken":"...","mfaRequired":true|false}` invece di un token di sessione. Invia quel `mfaToken` più un codice TOTP o di recupero a `/api/auth/mfa/complete`.
...
| Metodo | Percorso | Accesso | Descrizione |
|--------|------|--------|-------------|
| `GET` | `/api/v1/health` | Pubblico | Controllo di base dello stato. Restituisce `{"status":"healthy","version":"..."}` con 200, oppure `{"status":"unhealthy"}` con 503 se il database è irraggiungibile. |
| `GET` | `/api/v1/readyz` | Pubblico | Sonda di prontezza. Verifica PostgreSQL, Redis, lo spazio su disco e S3 quando configurato. Restituisce 503 quando l'istanza non deve ricevere traffico. |
...
## Uso degli strumenti {#using-tools}
...
- `clientJobId` è un campo del modulo opzionale per la correlazione dell'avanzamento fornita dal chiamante.
- `fileId` è un campo del modulo opzionale che fa riferimento a un elemento esistente della libreria dei file. Quando è presente, l'output elaborato viene salvato come nuova versione e la risposta include `savedFileId`.
- **Strumenti veloci** di solito restituiscono JSON 200: `{"jobId":"...","downloadUrl":"/api/v1/download/<jobId>/<filename>","originalSize":1234,"processedSize":567}`. Recupera il file elaborato da `downloadUrl`.
- **Qualsiasi strumento in coda** può restituire JSON 202 se è a lunga esecuzione o supera la finestra di attesa sincrona: `{"jobId":"...","async":true}`. Connettiti al flusso SSE per l'avanzamento, poi scarica al completamento (vedi [Monitoraggio dell'avanzamento](#progress-tracking)).
- Le route **batch** restituiscono un archivio ZIP trasmesso in streaming direttamente (con l'header `X-Job-Id`) per gli strumenti registrati nel registro batch generico.

## Riferimento degli strumenti {#tools-reference}

### Preset di conversione {#conversion-presets}

Il catalogo condiviso include 83 endpoint dedicati ai preset di conversione, come `jpg-to-png`, `mov-to-mp4`, `m4a-to-mp3`, `pdf-to-jpg` e `excel-to-csv`. I preset sono route di strumenti di prima classe:

`POST /api/v1/tools/<section>/<presetId>`

Ogni preset blocca il formato di output e delega a uno strumento di base come `convert`, `convert-video`, `extract-audio`, `convert-audio`, `image-to-pdf`, `pdf-to-image`, `svg-to-raster` o `convert-spreadsheet`. Vedi [Preset di conversione](/it/tools/conversion-presets) per la tabella completa delle route e le impostazioni opzionali.

### Elementi essenziali {#essentials}

| ID strumento | Nome | Impostazioni chiave |
|---------|------|-------------|
...
| `crop` | Ritaglia | `left`, `top`, `width`, `height`, `unit` (px/percentuale) |
...
| `strip-metadata` | Rimuovi i metadati | - |
| `edit-metadata` | Modifica i metadati | `title`, `description`, `author`, `copyright`, `keywords`, `gps` (lat/lon), `dateTime` |
...
| `replace-color` | Sostituisci il colore | `sourceColor`, `targetColor` (sostituto), `makeTransparent`, `tolerance` |
...
### Strumenti IA {#ai-tools}

Tutti gli strumenti IA vengono eseguiti sul tuo hardware: CPU per impostazione predefinita, oppure NVIDIA CUDA quando è disponibile una GPU NVIDIA supportata. L'accelerazione tramite iGPU Intel/AMD attraverso VA-API, Quick Sync o OpenCL non è supportata oggi per l'inferenza IA. Non è richiesta alcuna connessione a internet.

| ID strumento | Nome | Modello IA | Impostazioni chiave |
|---------|------|---------|-------------|
| `remove-background` | Rimuovi lo sfondo | rembg (BiRefNet / U2-Net) | `model`, `backgroundType` (transparent/color/gradient/blur/image), `backgroundColor`, `gradientColor1`, `gradientColor2`, `gradientAngle`, `blurEnabled`, `blurIntensity`, `shadowEnabled`, `shadowOpacity` |
| `upscale` | Upscaling delle immagini | RealESRGAN | `scale` (2/4), `model`, `faceEnhance`, `denoise`, `format`, `quality` |
| `erase-object` | Gomma per oggetti | LaMa (ONNX) | Maschera inviata come seconda parte di file (nome del campo `mask`), `format`, `quality` |
| `ocr` | OCR / Estrazione del testo | PaddleOCR / Tesseract | `quality` (fast/balanced/best), `language`, `enhance` |
| `blur-faces` | Sfocatura volto / PII | MediaPipe | `blurRadius`, `sensitivity` |
...
| `image-enhancement` | Miglioramento dell'immagine | Basato sull'analisi | `mode` (auto/exposure/contrast/color/sharpness), `strength` |
| `enhance-faces` | Miglioramento del volto | GFPGAN / CodeFormer | `model` (gfpgan/codeformer), `strength`, `sensitivity`, `centerFace` |
| `colorize` | Colorizzazione IA | DDColor | `intensity`, `model` |
| `noise-removal` | Rimozione del rumore | Denoising a livelli | `tier` (quick/balanced/quality/maximum), `strength`, `detailPreservation`, `colorNoise`, `format`, `quality` |
| `red-eye-removal` | Rimozione occhi rossi | Punti di riferimento del volto + analisi del colore | `sensitivity`, `strength` |
| `restore-photo` | Restauro fotografico | Pipeline multi-fase | `mode` (auto/light/heavy), `scratchRemoval`, `faceEnhancement`, `fidelity`, `denoise`, `denoiseStrength`, `colorize` |
| `passport-photo` | Foto tessera | Punti di riferimento MediaPipe | Flusso a due fasi. L'analisi usa multipart `file`; la generazione usa JSON con `countryCode`, `bgColor`, `printLayout` (none/4x6/a4), punti di riferimento, dimensioni dell'immagine |
| `content-aware-resize` | Ridimensionamento con riconoscimento dei contenuti | Seam carving (caire) | `width`, `height`, `protectFaces`, `blurRadius`, `sobelThreshold`, `square` |
| `transparency-fixer` | Correttore di trasparenza PNG | BiRefNet HR-matting | `defringe` (0-100), `outputFormat` (png/webp) |
| `background-replace` | Sostituisci lo sfondo | rembg (BiRefNet) | `backgroundType` (color/gradient), `color` (hex), `gradientColor1`, `gradientColor2`, `gradientAngle`, `feather` (0-20), `format` (png/webp) |
| `blur-background` | Sfoca lo sfondo | rembg (BiRefNet) | `intensity` (1-100), `feather` (0-20), `format` (png/webp) |
| `ai-canvas-expand` | Espansione IA della tela | LaMa (outpainting) | `extendTop`, `extendRight`, `extendBottom`, `extendLeft` (px), `tier` (fast/balanced/high), `format`, `quality` |
...
| `watermark-image` | Filigrana con immagine | `opacity`, `position`, `scale` - il secondo file è la filigrana |
| `text-overlay` | Sovrapposizione di testo | `text`, `font`, `fontSize`, `color`, `x`, `y`, `background`, `padding`, `borderRadius` |
| `compose` | Composizione di immagini | `x`, `y`, `opacity`, `blend` - il secondo file viene sovrapposto in alto |
...
| ID strumento | Nome | Impostazioni chiave |
|---------|------|-------------|
| `info` | Informazioni sull'immagine | - (restituisce larghezza, altezza, formato, dimensione, canali, hasAlpha, DPI, EXIF) |
| `compare` | Confronta immagini | `mode` (side-by-side/overlay/diff), `diffThreshold` - il secondo file è l'obiettivo del confronto |
...
| `qr-generate` | Generatore di codici QR | `data`, `size`, `margin`, `colorDark`, `colorLight`, `errorCorrectionLevel`, `dotStyle`, `cornerStyle`, `logo` (file opzionale) |
...
| `histogram` | Istogramma | `scale` (linear/log) - restituisce un grafico dell'istogramma RGB + statistiche per canale |
...
| ID strumento | Nome | Impostazioni chiave |
|---------|------|-------------|
| `collage` | Collage / Griglia | `template` (oltre 25 layout), `gap`, `backgroundColor`, `borderRadius` - multi-file |
| `stitch` | Unisci / Combina | `direction` (horizontal/vertical/grid), `gap`, `backgroundColor`, `alignment` - multi-file |
| `split` | Suddivisione dell'immagine | `mode` (grid/rows/cols), `rows`, `cols`, `tileWidth`, `tileHeight` |
...
| `image-pad` | Riempimento dell'immagine | `target` (16:9/9:16/1:1/4:3/3:4/custom), `ratioW`, `ratioH`, `background` (color/transparent/blur), `color` (hex), `padding` (0-50%) |
| `sprite-sheet` | Sprite sheet | `columns` (1-16), `padding`, `background` (hex), `format` (png/webp/jpeg), `quality` - multi-file (2-64 immagini) |
...
| `aspect-pad` | Riempimento proporzioni | `target` (16:9/9:16/1:1/4:3/3:4), `color` (hex) |
| `blur-pad` | Riempimento sfocato | `target` (16:9/9:16/1:1/4:3/3:4), `blur` (2-50) |
...
| `merge-videos` | Unisci video | - (multi-file, normalizzati alla risoluzione del primo video) |
| `replace-audio` | Sostituisci audio | - (file video + audio, due file) |
| `burn-subtitles` | Incorpora sottotitoli (burn) | `fontSize` (8-72) - file video + sottotitoli |
| `embed-subtitles` | Incorpora sottotitoli | `language` (codice ISO 639-2/B) - file video + sottotitoli |
...
| `video-metadata` | Pulisci i metadati del video | - |
| `auto-subtitles` | Sottotitoli automatici (IA) | `language` (auto/en/de/fr/es/zh/ja/ko/id/th/vi), `format` (srt/vtt) |
...
| `volume-adjust` | Regola il volume | `gainDb` (da -30 a 30) |
...
| `pitch-shift` | Cambio di intonazione | `semitones` (da -12 a 12) |
| `audio-channels` | Canali audio | `mode` (stereo-to-mono/mono-to-stereo/swap) |
| `silence-removal` | Rimozione del silenzio | `thresholdDb` (da -80 a -20), `minSilenceS` (0.1-5) |
...
| `transcribe-audio` | Trascrivi audio (IA) | `language` (auto/en/de/fr/es/zh/ja/ko/id/th/vi), `outputFormat` (txt/srt/vtt) |
...
| `extract-pages` | Estrai pagine | `range` (sintassi qpdf, ad es. "1-5,8,10-z") |
| `remove-pages` | Rimuovi pagine | `pages` (intervallo qpdf da rimuovere) |
| `organize-pdf` | Organizza PDF | `order` (ordine delle pagine qpdf, ad es. "3,1,2,5-z") |
...
| `pdfa-convert` | Converti in PDF/A | - (PDF/A-2 per l'archiviazione) |
...
| `flatten-pdf` | Appiattisci PDF | - (fissa moduli e annotazioni) |
...
| `extract-zip` | Estrai ZIP | - (protetto da bomb) |
...
Alcuni strumenti espongono endpoint aggiuntivi oltre a quello standard `POST /api/v1/tools/<section>/<toolId>`:

| Metodo | Percorso | Descrizione |
|--------|------|-------------|
| `GET` | `/api/v1/tools/popular` | Restituisce gli ID degli strumenti più popolari, ricadendo su un elenco predefinito curato quando i dati di utilizzo sono scarsi |
| `POST` | `/api/v1/tools/image/remove-background/effects` | Applica effetti allo sfondo (color/gradient/blur/shadow) senza rieseguire l'IA. Usa la maschera in cache della rimozione iniziale. |
...
| `POST` | `/api/v1/tools/image/passport-photo/analyze` | Fase 1: rilevamento IA del volto + rimozione dello sfondo. Restituisce i punti di riferimento del volto e i dati in cache. |
| `POST` | `/api/v1/tools/image/passport-photo/generate` | Fase 2: ritaglia, ridimensiona e componi affiancando usando l'analisi in cache. Nessuna riesecuzione dell'IA. |
| `POST` | `/api/v1/tools/image/gif-tools/info` | Ottieni i metadati della GIF (numero di frame, dimensioni, durata) |
| `POST` | `/api/v1/tools/pdf/pdf-to-image/info` | Ottieni i metadati del PDF (numero di pagine, dimensioni) |
| `POST` | `/api/v1/tools/pdf/pdf-to-image/preview` | Genera un'anteprima di una pagina PDF specifica |
| `POST` | `/api/v1/tools/pdf/pdf-to-jpg/info` | Ottieni i metadati del PDF per il preset JPG dedicato |
| `POST` | `/api/v1/tools/pdf/pdf-to-jpg/preview` | Genera un'anteprima di pagina PDF con il preset JPG |
| `POST` | `/api/v1/tools/pdf/pdf-to-png/info` | Ottieni i metadati del PDF per il preset PNG dedicato |
| `POST` | `/api/v1/tools/pdf/pdf-to-png/preview` | Genera un'anteprima di pagina PDF con il preset PNG |
| `POST` | `/api/v1/tools/pdf/pdf-to-tiff/info` | Ottieni i metadati del PDF per il preset TIFF dedicato |
| `POST` | `/api/v1/tools/pdf/pdf-to-tiff/preview` | Genera un'anteprima di pagina PDF con il preset TIFF |
| `POST` | `/api/v1/tools/image/svg-to-raster/batch` | Converti in batch più SVG in raster |
| `POST` | `/api/v1/tools/image/image-enhancement/analyze` | Analizza la qualità dell'immagine e restituisci raccomandazioni di miglioramento |
| `POST` | `/api/v1/tools/image/optimize-for-web/preview` | Anteprima leggera per la messa a punto dei parametri in tempo reale. Restituisce un'immagine ottimizzata con header di dimensione. |
...
### Eseguire una pipeline {#execute-a-pipeline}
...
L'output di ogni passaggio è l'input del passaggio successivo. Le pipeline consentono 20 passaggi per impostazione predefinita, configurabili tramite `MAX_PIPELINE_STEPS`. Imposta `MAX_PIPELINE_STEPS=0` per rimuovere il limite.

### Salvare e gestire le pipeline {#save-and-manage-pipelines}
...
| `GET` | `/api/v1/pipeline/tools` | Elenca gli ID degli strumenti validi per i passaggi della pipeline |

## Monitoraggio dell'avanzamento {#progress-tracking}

I job a lunga esecuzione, gli strumenti in coda, i job in batch e le pipeline emettono l'avanzamento in tempo reale tramite Server-Sent Events. Il flusso di avanzamento è pubblico e indicizzato per ID del job, quindi i client non devono inviare un header Authorization per leggerlo.
...
| Metodo | Percorso | Descrizione |
|--------|------|-------------|
| `POST` | `/api/v1/upload` | Carica file nell'area di lavoro (elaborazione temporanea) |
| `POST` | `/api/v1/files/upload` | Carica file nella libreria dei file persistente |
| `POST` | `/api/v1/files/save-result` | Salva il risultato dell'elaborazione di uno strumento come nuova versione del file |
| `GET` | `/api/v1/files` | Elenca i file salvati (impaginati, con ricerca) |
| `GET` | `/api/v1/files/:id` | Ottieni i metadati del file + la catena delle versioni |
| `GET` | `/api/v1/files/:id/download` | Scarica il file |
| `GET` | `/api/v1/files/:id/thumbnail` | Ottieni una miniatura JPEG da 300px |
| `DELETE` | `/api/v1/files` | Elimina in blocco i file e le loro catene di versioni (corpo: `{ ids: [...] }`) |
| `POST` | `/api/v1/fetch-urls` | Recupera URL remoti nell'area di lavoro per le importazioni basate su URL |
| `POST` | `/api/v1/preview` | Genera un'anteprima WebP compatibile con il browser (per i formati HEIC/HEIF/RAW) |
| `GET` | `/api/v1/files/:id/preview` | Trasmetti in streaming un'anteprima compatibile con il browser, in cache o generata, per un PDF, un documento Office, un video o un file audio salvato |
...
| `GET` | `/api/v1/download/:jobId/:filename` | Scarica un file elaborato da un'area di lavoro |
...
Configurazione chiave-valore in fase di esecuzione (leggibile da qualsiasi utente autenticato, scrivibile solo dall'amministratore).
...
| `GET` | `/api/v1/settings/:key` | Ottieni un'impostazione specifica per chiave |

Chiavi note: `disabledTools` (array JSON di ID di strumenti), `enableExperimentalTools` (stringa bool), `loginAttemptLimit` (numero).
...
| Metodo | Percorso | Descrizione |
|--------|------|-------------|
| `GET` | `/api/v1/preferences` | Ottieni le preferenze dell'utente corrente come `{ "preferences": { ... } }` |
| `PUT` | `/api/v1/preferences` | Inserisci o aggiorna una o più chiavi di preferenza per l'utente corrente |
...
| Metodo | Percorso | Accesso | Descrizione |
|--------|------|--------|-------------|
| `GET` | `/api/v1/audit-log` | Admin (`audit:read`) | Registro di controllo impaginato con filtri opzionali |
...
| `action` | Filtra per tipo di azione (ad es. `ROLE_CREATED`, `ROLE_DELETED`) |
| `ip` | Filtra per indirizzo IP di origine |
| `from` | Filtra le voci successive a questa data ISO 8601 |
| `to` | Filtra le voci precedenti a questa data ISO 8601 |

## Analytics {#analytics}

| Metodo | Percorso | Accesso | Descrizione |
|--------|------|--------|-------------|
| `GET` | `/api/v1/config/analytics` | Pubblico | Ottieni la configurazione effettiva degli analytics (chiave PostHog, DSN Sentry, tasso di campionamento). Le chiavi, il DSN e l'ID dell'istanza sono vuoti quando gli analytics sono disattivati, sia dal bake in fase di compilazione sia dall'impostazione dell'istanza `analyticsEnabled`. |
| `POST` | `/api/v1/feedback` | Auth | Invia il feedback esplicito dell'utente al progetto PostHog configurato come `feedback_submitted`. La route rispetta il gate degli analytics, limita la frequenza degli invii, rimuove i campi di contatto a meno che `contactOk` non sia true, e non accetta mai il contenuto dei file, i nomi dei file, i percorsi di caricamento o il testo grezzo di errori privati. Quando gli analytics sono disabilitati, restituisce `{ "ok": true, "accepted": false }`. |
...
## Funzionalità / Bundle IA {#features-ai-bundles}

Gestisci i bundle delle funzionalità IA (installa/disinstalla i pacchetti dei modelli IA nell'ambiente Docker).

| Metodo | Percorso | Accesso | Descrizione |
|--------|------|--------|-------------|
| `GET` | `/api/v1/features` | Auth | Elenca tutti i bundle delle funzionalità e il loro stato di installazione |
| `POST` | `/api/v1/admin/features/:bundleId/install` | Admin (`features:manage`) | Installa un bundle di funzionalità (asincrono, restituisce `jobId` per il monitoraggio dell'avanzamento) |
| `POST` | `/api/v1/admin/features/:bundleId/uninstall` | Admin (`features:manage`) | Disinstalla un bundle di funzionalità e ripulisce i file dei modelli |
| `GET` | `/api/v1/admin/features/disk-usage` | Admin (`features:manage`) | Ottieni l'utilizzo totale del disco dei modelli IA |
| `POST` | `/api/v1/admin/features/import` | Admin (`features:manage`) | Importa un archivio di bundle IA offline |
...
| Metodo | Percorso | Accesso | Descrizione |
|--------|------|--------|-------------|
| `GET` | `/api/v1/admin/log-level` | Admin (`settings:write`) | Leggi il livello di log corrente in fase di esecuzione |
| `POST` | `/api/v1/admin/log-level` | Admin (`settings:write`) | Cambia il livello di log in fase di esecuzione (`fatal`, `error`, `warn`, `info`, `debug`, `trace` o `silent`) |
| `GET` | `/api/v1/metrics` | Admin (`system:health`) | Metriche Prometheus in formato testuale |
| `GET` | `/api/v1/admin/support-bundle` | Admin (`system:health`) | Scarica un bundle ZIP di supporto diagnostico oscurato |
| `GET` | `/api/v1/admin/usage` | Admin (`audit:read`) | Dati del cruscotto di utilizzo, con parametro della query `days` opzionale |
| `GET` | `/api/v1/admin/backup-status` | Admin (`system:health`) | Leggi i metadati dell'ultimo backup e lo stato di aggiornamento |
| `POST` | `/api/v1/admin/backup-status` | Admin (`system:health`) | Registra un backup completato (`type`, `sizeBytes` opzionale, `notes` opzionale) |

## API Enterprise {#enterprise-apis}

Queste route sono soggette a licenza in base alla relativa funzionalità enterprise. Richiedono comunque il permesso SnapOtter elencato.

| Metodo | Percorso | Accesso | Descrizione |
|--------|------|--------|-------------|
| `GET` | `/api/v1/enterprise/audit/export` | Admin (`audit:read`) | Esporta le voci del registro di controllo come JSON o CSV con filtri |
| `GET` | `/api/v1/enterprise/config/export` | Admin (`system:health`) | Esporta la configurazione oscurata dell'istanza, i ruoli personalizzati e i team |
| `POST` | `/api/v1/enterprise/config/import` | Admin (`system:health`) | Importa la configurazione, con esecuzione a vuoto opzionale |
| `GET` | `/api/v1/enterprise/ip-allowlist` | Admin (`security:manage`) | Leggi l'elenco di autorizzazione CIDR configurato |
| `PUT` | `/api/v1/enterprise/ip-allowlist` | Admin (`security:manage`) | Aggiorna l'elenco di autorizzazione CIDR con prevenzione dell'auto-blocco |
...
| `GET` | `/api/v1/enterprise/siem/config` | Admin (`webhooks:manage`) | Leggi la configurazione dell'inoltro SIEM |
...
| `GET` | `/api/v1/enterprise/users/:id/export/:jobId` | Admin (`compliance:manage`) | Leggi lo stato dell'esportazione GDPR e l'URL di download |
...
| `GET` | `/api/v1/admin/version` | Admin (`system:health`) | Leggi i metadati di versione dell'app, della build, di Node e dello schema |
| `GET` | `/api/v1/admin/migrations/pending` | Admin (`system:health`) | Confronta le migrazioni impacchettate con quelle applicate |
| `GET` | `/api/v1/admin/upgrade-check` | Admin (`system:health`) | Esegui i controlli di prontezza per l'aggiornamento |

### SCIM 2.0 {#scim-2-0}

Gli endpoint di discovery SCIM sono pubblici. Gli endpoint per utenti e gruppi richiedono il token bearer SCIM generato sopra.

| Metodo | Percorso | Accesso | Descrizione |
|--------|------|--------|-------------|
| `GET` | `/api/v1/scim/v2/ServiceProviderConfig` | Pubblico | Funzionalità del server SCIM |
...
| `GET` | `/api/v1/scim/v2/Users` | Token SCIM | Elenca gli utenti, con filtro SCIM opzionale |
| `POST` | `/api/v1/scim/v2/Users` | Token SCIM | Crea un utente |
| `GET` | `/api/v1/scim/v2/Users/:id` | Token SCIM | Ottieni un utente |
| `PUT` | `/api/v1/scim/v2/Users/:id` | Token SCIM | Sostituisci un utente |
...
| `GET` | `/api/v1/scim/v2/Groups/:id` | Token SCIM | Ottieni un team |
| `PUT` | `/api/v1/scim/v2/Groups/:id` | Token SCIM | Sostituisci un team e l'appartenenza al gruppo |
| `DELETE` | `/api/v1/scim/v2/Groups/:id` | Token SCIM | Elimina un team |

## Template di meme {#meme-templates}

API di supporto per lo strumento generatore di meme.

| Metodo | Percorso | Accesso | Descrizione |
|--------|------|--------|-------------|
| `GET` | `/api/v1/meme-templates` | Auth | Elenca tutti i template di meme disponibili con le posizioni dei riquadri di testo |
| `GET` | `/api/v1/meme-templates/full/:filename` | Auth | Fornisci l'immagine del template a dimensione piena |
| `GET` | `/api/v1/meme-templates/thumbs/:filename` | Auth | Fornisci la miniatura del template |
| `GET` | `/api/v1/meme-templates/fonts/:filename` | Auth | Fornisci il file del font usato per il rendering del testo dei meme |
...
| 501 | Il bundle di funzionalità IA richiesto non è installato (`FEATURE_NOT_INSTALLED`) |
