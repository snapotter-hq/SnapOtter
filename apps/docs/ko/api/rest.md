---
description: "완전한 REST API 참조. 도구 엔드포인트, 일괄 처리, 파이프라인, 파일 라이브러리, 인증, 팀, 관리자 작업."
i18n_source_hash: eb73a14533a1
i18n_provenance: human
i18n_output_hash: ef8e707df03a
---
...
기계 판독용 명세:
- `/api/v1/openapi.yaml` - OpenAPI 3.1 명세
- `/llms.txt` - LLM 친화적 요약
- `/llms-full.txt` - LLM 친화적 전체 문서
...
세션은 7일 후 만료됩니다(`SESSION_DURATION_HOURS`로 설정 가능).
...
키는 `si_` 접두어가 붙으며 scrypt 해시로 저장됩니다. 원본 키는 한 번만 표시되며 이후에는 다시 조회할 수 없습니다.
...
| `POST` | `/api/auth/logout` | 인증 필요 | 현재 세션 파기 |
| `GET` | `/api/auth/session` | 인증 필요 | 현재 세션 검증 |
| `POST` | `/api/auth/change-password` | 인증 필요 | 본인 비밀번호 변경(다른 모든 세션 및 API 키 무효화) |
...
| `POST` | `/api/auth/mfa/enroll` | 인증 필요 | TOTP MFA 등록 시작. 엔터프라이즈 `mfa` 기능이 필요합니다 |
| `POST` | `/api/auth/mfa/verify` | 인증 필요 | TOTP 코드로 MFA 등록 확인 |
| `POST` | `/api/auth/mfa/complete` | 공개 | 대기 중인 MFA 로그인 챌린지 완료 |
| `POST` | `/api/auth/mfa/disable` | 인증 필요 | 현재 사용자의 MFA 비활성화 |
| `POST` | `/api/auth/users/:id/mfa/reset` | 관리자(`users:manage`) | 사용자의 MFA 재설정 |
| `GET` | `/api/auth/oidc/login` | 공개 | OIDC가 활성화된 경우 OIDC 로그인 시작 |
| `GET` | `/api/auth/oidc/callback` | 공개 | OIDC 인가 콜백 |
| `GET` | `/api/auth/saml/metadata` | 공개 | SAML이 활성화된 경우 SAML SP 메타데이터 XML |
| `GET` | `/api/auth/saml/login` | 공개 | SAML 로그인 시작 |
| `POST` | `/api/auth/saml/callback` | 공개 | SAML 어서션 컨슈머 서비스 |

사용자에게 MFA가 활성화되면 `POST /api/auth/login`는 세션 토큰 대신 `{"requiresMfa":true,"mfaToken":"...","mfaRequired":true|false}`을 반환합니다. 해당 `mfaToken`과 TOTP 또는 복구 코드를 `/api/auth/mfa/complete`로 전송하세요.
...
| 메서드 | 경로 | 접근 권한 | 설명 |
|--------|------|--------|-------------|
| `GET` | `/api/v1/health` | 공개 | 기본 상태 확인. 정상이면 200과 함께 `{"status":"healthy","version":"..."}`를, 데이터베이스에 연결할 수 없으면 503과 함께 `{"status":"unhealthy"}`을 반환합니다. |
| `GET` | `/api/v1/readyz` | 공개 | 준비 상태 프로브. PostgreSQL, Redis, 디스크 공간, 그리고 구성된 경우 S3를 확인합니다. 인스턴스가 트래픽을 받으면 안 되는 경우 503을 반환합니다. |
| `GET` | `/api/v1/admin/health` | 관리자(`system:health`) | 가동 시간, 스토리지 모드, 데이터베이스 상태, 큐 상태, GPU 가용성을 포함한 상세 진단 정보. |
...
- `settings`는 도구별 옵션이 담긴 JSON 문자열입니다.
- `clientJobId`은 호출자가 제공하는 진행률 상관 관계를 위한 선택적 폼 필드입니다.
- `fileId`은 기존 파일 라이브러리 항목을 참조하는 선택적 폼 필드입니다. 이 값이 있으면 처리된 출력이 새 버전으로 저장되고 응답에 `savedFileId`가 포함됩니다.
...
- **큐에 등록된 모든 도구**는 장시간 실행되거나 동기 대기 시간을 초과하는 경우 202 JSON을 반환할 수 있습니다: `{"jobId":"...","async":true}`. 진행률을 위해 SSE에 연결한 뒤 완료되면 다운로드하세요([진행률 추적](#progress-tracking) 참조).
- **일괄** 라우트는 일반 일괄 레지스트리에 등록된 도구에 대해 ZIP 아카이브를 직접 스트리밍하여 반환합니다(`X-Job-Id` 헤더 포함).

## 도구 참조 {#tools-reference}

### 변환 프리셋 {#conversion-presets}

공유 카탈로그에는 `jpg-to-png`, `mov-to-mp4`, `m4a-to-mp3`, `pdf-to-jpg`, `excel-to-csv`와 같은 전용 변환 프리셋 엔드포인트 83개가 포함되어 있습니다. 프리셋은 일급 도구 라우트입니다:

`POST /api/v1/tools/<section>/<presetId>`

각 프리셋은 출력 형식을 고정하고 `convert`, `convert-video`, `extract-audio`, `convert-audio`, `image-to-pdf`, `pdf-to-image`, `svg-to-raster`, `convert-spreadsheet`과 같은 기반 도구에 위임합니다. 전체 라우트 표와 선택적 설정은 [변환 프리셋](/ko/tools/conversion-presets)을 참조하세요.

### 필수 도구 {#essentials}

| 도구 ID | 이름 | 주요 설정 |
|---------|------|-------------|
| `resize` | 크기 조정 | `width`, `height`, `fit`(cover/contain/fill/inside/outside), `percentage`, `withoutEnlargement`, 그리고 소셜 미디어 프리셋 23종 |
...
| `favicon` | 파비콘 생성기 | `padding`, `backgroundColor`, `borderRadius` - 모든 표준 크기 생성 |
...
| `sharpening` | 샤프닝 | `method`(adaptive/unsharp-mask/high-pass), `sigma`, `m1`, `m2`, `x1`, `y2`, `y3`, `amount`, `radius`, `threshold`, `strength`, `kernelSize`(3/5), `denoise`(off/light/medium/strong) |
| `replace-color` | 색상 교체 | `sourceColor`, `targetColor`(교체 대상), `makeTransparent`, `tolerance` |
...
모든 AI 도구는 사용자의 하드웨어에서 실행됩니다: 기본적으로 CPU를 사용하며, 지원되는 NVIDIA GPU가 있으면 NVIDIA CUDA를 사용합니다. VA-API, Quick Sync, OpenCL을 통한 Intel/AMD iGPU 가속은 현재 AI 추론에 지원되지 않습니다. 인터넷이 필요하지 않습니다.
...
| `upscale` | 이미지 업스케일링 | RealESRGAN | `scale`(2/4), `model`, `faceEnhance`, `denoise`, `format`, `quality` |
| `erase-object` | 개체 지우개 | LaMa (ONNX) | 마스크는 두 번째 파일 파트로 전송(필드명 `mask`), `format`, `quality` |
| `ocr` | OCR / 텍스트 추출 | PaddleOCR / Tesseract | `quality`(fast/balanced/best), `language`, `enhance` |
| `blur-faces` | 얼굴 / PII 흐림 | MediaPipe | `blurRadius`, `sensitivity` |
...
| `colorize` | AI 컬러화 | DDColor | `intensity`, `model` |
| `noise-removal` | 노이즈 제거 | 단계별 노이즈 제거 | `tier`(quick/balanced/quality/maximum), `strength`, `detailPreservation`, `colorNoise`, `format`, `quality` |
...
| `passport-photo` | 여권 사진 | MediaPipe 랜드마크 | 2단계 흐름. 분석은 멀티파트 `file`을 사용하고, 생성은 `countryCode`, `bgColor`, `printLayout`(none/4x6/a4), 랜드마크, 이미지 치수가 담긴 JSON을 사용합니다 |
| `content-aware-resize` | 콘텐츠 인식 크기 조정 | 심 카빙(caire) | `width`, `height`, `protectFaces`, `blurRadius`, `sobelThreshold`, `square` |
| `transparency-fixer` | PNG 투명도 보정기 | BiRefNet HR-matting | `defringe`(0-100), `outputFormat`(png/webp) |
...
| `blur-background` | 배경 흐림 | rembg (BiRefNet) | `intensity`(1-100), `feather`(0-20), `format`(png/webp) |
...
| `compare` | 이미지 비교 | `mode`(side-by-side/overlay/diff), `diffThreshold` - 두 번째 파일이 비교 대상 |
| `find-duplicates` | 중복 찾기 | `threshold`(지각 해시 거리, 기본값 8) - 다중 파일 |
...
| `barcode-read` | 바코드 리더 | - (QR, EAN, Code128, DataMatrix 등 자동 감지) |
...
| 도구 ID | 이름 | 주요 설정 |
|---------|------|-------------|
| `collage` | 콜라주 / 그리드 | `template`(25개 이상 레이아웃), `gap`, `backgroundColor`, `borderRadius` - 다중 파일 |
...
| `sprite-sheet` | 스프라이트 시트 | `columns`(1-16), `padding`, `background`(hex), `format`(png/webp/jpeg), `quality` - 다중 파일(이미지 2-64개) |
...
| `gif-tools` | GIF 도구 | `action`(resize/optimize/reverse/speed/extract-frames/rotate/add-text), 작업별 파라미터 |
...
| `trim-video` | 비디오 자르기 | `startS`, `endS`, `precise`(bool, 프레임 정확도 컷) |
| `mute-video` | 비디오 음소거 | - |
...
| `blur-pad` | 흐림 패딩 | `target`(16:9/9:16/1:1/4:3/3:4), `blur`(2-50) |
...
| `merge-videos` | 비디오 병합 | - (다중 파일, 첫 번째 비디오 해상도로 정규화) |
| `replace-audio` | 오디오 교체 | - (비디오 + 오디오 파일, 두 파일) |
...
| `repair-pdf` | PDF 복구 | - |
| `linearize-pdf` | PDF 웹 최적화 | - (빠른 웹 열람을 위한 선형화) |
| `grayscale-pdf` | PDF 회색조 | - |
| `pdfa-convert` | PDF/A 변환 | - (보관용 PDF/A-2) |
...
| `flatten-pdf` | PDF 평탄화 | - (양식 및 주석을 병합) |
| `redact-pdf` | PDF 편집 삭제 | `terms`(string[]), `caseSensitive`(bool) |
| `sign-pdf` | PDF 서명 | PDF `file`, 서명 파일 `sig0`, `sig1`, `placements` JSON 배열을 사용하는 사용자 지정 멀티파트 라우트 |
...
| `merge-csvs` | CSV 병합 | - (다중 파일, 열 일치) |
| `yaml-json` | YAML / JSON | - (양방향) |
| `xml-to-csv` | XML을 CSV로 | - (반복 요소 자동 탐색) |
| `excel-to-csv` | Excel을 CSV로 | `convert-spreadsheet`를 기반으로 하는 전용 변환 프리셋 |
| `create-zip` | ZIP 생성 | - (다중 파일, 2-50개 파일) |
| `extract-zip` | ZIP 추출 | - (압축 폭탄 방지) |

### HTML을 이미지로 {#html-to-image}

웹페이지를 이미지로 캡처합니다. 다른 도구와 달리 이 엔드포인트는 멀티파트 폼 데이터 대신 `application/json`을 받습니다(파일 업로드 불필요).
...
| 파라미터 | 타입 | 기본값 | 설명 |
|-----------|------|---------|-------------|
| `url` | string | (필수) | 캡처할 URL(http/https만 가능) |
| `format` | string | `"png"` | 출력 형식: `jpg`, `png`, `webp` |
| `quality` | number | `90` | 품질 1-100(JPG/WebP 전용) |
| `fullPage` | boolean | `false` | 스크롤 가능한 전체 페이지 캡처 |
| `devicePreset` | string | `"desktop"` | `desktop`, `tablet`, `mobile`, `custom` |
| `viewportWidth` | number | `1280` | 사용자 지정 뷰포트 너비 320-3840 |
| `viewportHeight` | number | `720` | 사용자 지정 뷰포트 높이 320-2160 |
...
### 도구 하위 라우트 {#tool-sub-routes}

일부 도구는 표준 `POST /api/v1/tools/<section>/<toolId>` 외에 추가 엔드포인트를 제공합니다:

| 메서드 | 경로 | 설명 |
|--------|------|-------------|
| `GET` | `/api/v1/tools/popular` | 인기 도구 ID를 반환하며, 사용 데이터가 부족하면 선별된 기본 목록으로 대체 |
| `POST` | `/api/v1/tools/image/remove-background/effects` | AI를 다시 실행하지 않고 배경 효과(color/gradient/blur/shadow)를 적용. 초기 제거에서 캐시된 마스크를 사용. |
| `POST` | `/api/v1/tools/image/edit-metadata/inspect` | 이미지에서 기존 EXIF/IPTC/XMP 메타데이터 읽기 |
| `POST` | `/api/v1/tools/image/strip-metadata/inspect` | 제거 전 메타데이터 필드 검사 |
| `POST` | `/api/v1/tools/image/passport-photo/analyze` | 1단계: AI 얼굴 감지 + 배경 제거. 얼굴 랜드마크와 캐시된 데이터 반환. |
| `POST` | `/api/v1/tools/image/passport-photo/generate` | 2단계: 캐시된 분석을 사용해 자르기, 크기 조정, 타일링. AI 재실행 없음. |
| `POST` | `/api/v1/tools/image/gif-tools/info` | GIF 메타데이터 조회(프레임 수, 치수, 재생 시간) |
| `POST` | `/api/v1/tools/pdf/pdf-to-image/info` | PDF 메타데이터 조회(페이지 수, 치수) |
| `POST` | `/api/v1/tools/pdf/pdf-to-image/preview` | 특정 PDF 페이지의 미리보기 생성 |
| `POST` | `/api/v1/tools/pdf/pdf-to-jpg/info` | 전용 JPG 프리셋용 PDF 메타데이터 조회 |
| `POST` | `/api/v1/tools/pdf/pdf-to-jpg/preview` | JPG 프리셋 PDF 페이지 미리보기 생성 |
| `POST` | `/api/v1/tools/pdf/pdf-to-png/info` | 전용 PNG 프리셋용 PDF 메타데이터 조회 |
| `POST` | `/api/v1/tools/pdf/pdf-to-png/preview` | PNG 프리셋 PDF 페이지 미리보기 생성 |
| `POST` | `/api/v1/tools/pdf/pdf-to-tiff/info` | 전용 TIFF 프리셋용 PDF 메타데이터 조회 |
...
| `POST` | `/api/v1/tools/image/image-enhancement/analyze` | 이미지 품질 분석 후 향상 권장 사항 반환 |
| `POST` | `/api/v1/tools/image/optimize-for-web/preview` | 실시간 파라미터 튜닝을 위한 경량 미리보기. 크기 헤더가 포함된 최적화 이미지 반환. |

## 일괄 처리 {#batch-processing}

일반 일괄 지원 도구를 여러 파일에 한 번에 적용합니다. ZIP 아카이브를 반환합니다. PDF 서명, PDF OCR, PDF를 이미지로 변환하는 프리셋 라우트와 같은 사용자 지정 다중 파일 또는 다단계 라우트는 일반 `/batch` 라우트 대신 자체 엔드포인트 규약을 사용합니다.
...
동시성은 `CONCURRENT_JOBS`으로 제어됩니다(기본값: CPU 코어에서 자동 감지). `MAX_BATCH_SIZE`은 일괄당 파일 수를 제한합니다(기본값: 100; 무제한은 0으로 설정).
...
각 단계의 출력이 다음 단계의 입력이 됩니다. 파이프라인은 기본적으로 20단계를 허용하며 `MAX_PIPELINE_STEPS`로 설정할 수 있습니다. 제한을 없애려면 `MAX_PIPELINE_STEPS=0`로 설정하세요.
...
| `GET` | `/api/v1/pipeline/list` | 저장된 파이프라인 목록(관리자는 전체 조회; 사용자는 본인 것만) |
| `DELETE` | `/api/v1/pipeline/:id` | 삭제(소유자 또는 관리자) |
| `GET` | `/api/v1/pipeline/tools` | 파이프라인 단계에 유효한 도구 ID 목록 |

## 진행률 추적 {#progress-tracking}

장시간 실행 작업, 큐에 등록된 도구, 일괄 작업, 파이프라인은 Server-Sent Events를 통해 실시간 진행률을 전송합니다. 진행률 스트림은 공개이며 작업 ID로 키가 지정되므로, 클라이언트는 이를 읽기 위해 Authorization 헤더를 보낼 필요가 없습니다.
...
`POST /api/v1/jobs/:jobId/cancel`로 큐에 등록되었거나 실행 중인 작업의 취소를 요청할 수 있습니다. 응답은 `{"canceled":true|false}`입니다.
...
| `GET` | `/api/v1/files` | 저장된 파일 목록(페이지네이션, 검색 포함) |
| `GET` | `/api/v1/files/:id` | 파일 메타데이터 + 버전 체인 조회 |
| `GET` | `/api/v1/files/:id/download` | 파일 다운로드 |
| `GET` | `/api/v1/files/:id/thumbnail` | 300px JPEG 썸네일 조회 |
| `DELETE` | `/api/v1/files` | 파일과 버전 체인 일괄 삭제(본문: `{ ids: [...] }`) |
| `POST` | `/api/v1/fetch-urls` | URL 기반 가져오기를 위해 원격 URL을 워크스페이스로 가져오기 |
| `POST` | `/api/v1/preview` | 브라우저 호환 WebP 미리보기 생성(HEIC/HEIF/RAW 형식용) |
| `GET` | `/api/v1/files/:id/preview` | 저장된 PDF, 오피스 문서, 비디오, 오디오 파일에 대해 캐시되었거나 생성된 브라우저 호환 미리보기 스트리밍 |
...
| 메서드 | 경로 | 접근 권한 | 설명 |
|--------|------|--------|-------------|
| `POST` | `/api/v1/api-keys` | 인증 필요 | 새 키 생성 - 한 번만 표시 |
| `GET` | `/api/v1/api-keys` | 인증 필요 | 키 목록(name, id, lastUsedAt - 원본 키 아님) |
| `DELETE` | `/api/v1/api-keys/:id` | 인증 필요 | 키 삭제 |
...
런타임 키-값 구성(인증된 사용자는 누구나 읽기, 쓰기는 관리자만).

| 메서드 | 경로 | 설명 |
|--------|------|-------------|
| `GET` | `/api/v1/settings` | 모든 설정 조회 |
| `PUT` | `/api/v1/settings` | 설정 일괄 업데이트(키-값 쌍이 담긴 JSON 본문) |
| `GET` | `/api/v1/settings/:key` | 키로 특정 설정 조회 |

알려진 키: `disabledTools`(도구 ID의 JSON 배열), `enableExperimentalTools`(bool 문자열), `loginAttemptLimit`(number).

## 환경설정 {#preferences}

사용자별 환경설정은 인스턴스 설정과 별개입니다. 인증된 사용자는 누구나 자신의 환경설정 맵을 읽고 업데이트할 수 있습니다.

| 메서드 | 경로 | 설명 |
|--------|------|-------------|
| `GET` | `/api/v1/preferences` | 현재 사용자의 환경설정을 `{ "preferences": { ... } }`으로 조회 |
| `PUT` | `/api/v1/preferences` | 현재 사용자의 환경설정 키를 하나 이상 삽입/업데이트 |

## 역할 {#roles}

세분화된 권한을 가진 사용자 지정 역할 관리.

| 메서드 | 경로 | 접근 권한 | 설명 |
|--------|------|--------|-------------|
| `GET` | `/api/v1/roles` | 관리자(`audit:read`) | 사용자 수와 함께 모든 역할 목록 조회 |
| `POST` | `/api/v1/roles` | 관리자(`security:manage`) | 사용자 지정 역할 생성(`name`, `description`, `permissions`) |
| `PUT` | `/api/v1/roles/:id` | 관리자(`security:manage`) | 사용자 지정 역할 업데이트(내장 역할은 수정 불가) |
| `DELETE` | `/api/v1/roles/:id` | 관리자(`security:manage`) | 사용자 지정 역할 삭제(내장 역할은 삭제 불가; 영향을 받는 사용자는 `user` 역할로 되돌아감) |
...
| 메서드 | 경로 | 접근 권한 | 설명 |
|--------|------|--------|-------------|
| `GET` | `/api/v1/audit-log` | 관리자(`audit:read`) | 선택적 필터가 있는 페이지네이션 감사 로그 |
...
| `action` | 작업 유형별 필터(예: `ROLE_CREATED`, `ROLE_DELETED`) |
| `ip` | 소스 IP 주소별 필터 |
| `from` | 이 ISO 8601 날짜 이후 항목 필터 |
| `to` | 이 ISO 8601 날짜 이전 항목 필터 |

## 분석 {#analytics}

| 메서드 | 경로 | 접근 권한 | 설명 |
|--------|------|--------|-------------|
| `GET` | `/api/v1/config/analytics` | 공개 | 유효한 분석 구성 조회(PostHog 키, Sentry DSN, 샘플링 비율). 컴파일 타임 베이크 또는 인스턴스 `analyticsEnabled` 설정으로 분석이 꺼져 있으면 키, DSN, 인스턴스 ID가 비어 있습니다. |
| `POST` | `/api/v1/feedback` | 인증 필요 | 명시적 사용자 피드백을 구성된 PostHog 프로젝트에 `feedback_submitted`로 제출. 이 라우트는 분석 게이트를 준수하고, 제출 속도를 제한하며, `contactOk`가 true가 아닌 한 연락처 필드를 제거하고, 파일 내용, 파일명, 업로드 경로, 원본 비공개 오류 텍스트는 절대 받지 않습니다. 분석이 비활성화된 경우 `{ "ok": true, "accepted": false }`를 반환합니다. |
| `PUT` | `/api/v1/settings` | 관리자(`settings:write`) | 인스턴스 전역 옵트아웃 설정. 모두를 대상으로 분석을 끄려면 JSON 본문 `{ "analyticsEnabled": "false" }`을, 다시 켜려면 `"true"`을 전송하세요. |

## 기능 / AI 번들 {#features-ai-bundles}

AI 기능 번들 관리(Docker 환경에서 AI 모델 패키지 설치/제거).

| 메서드 | 경로 | 접근 권한 | 설명 |
|--------|------|--------|-------------|
| `GET` | `/api/v1/features` | 인증 필요 | 모든 기능 번들과 설치 상태 목록 조회 |
| `POST` | `/api/v1/admin/features/:bundleId/install` | 관리자(`features:manage`) | 기능 번들 설치(비동기, 진행률 추적용 `jobId` 반환) |
| `POST` | `/api/v1/admin/features/:bundleId/uninstall` | 관리자(`features:manage`) | 기능 번들 제거 및 모델 파일 정리 |
| `GET` | `/api/v1/admin/features/disk-usage` | 관리자(`features:manage`) | AI 모델의 총 디스크 사용량 조회 |
| `POST` | `/api/v1/admin/features/import` | 관리자(`features:manage`) | 오프라인 AI 번들 아카이브 가져오기 |

## 관리자 작업 {#admin-operations}

관측성, 지원, 사용량 보고, 백업 상태를 위한 운영 엔드포인트.

| 메서드 | 경로 | 접근 권한 | 설명 |
|--------|------|--------|-------------|
| `GET` | `/api/v1/admin/log-level` | 관리자(`settings:write`) | 현재 런타임 로그 수준 읽기 |
| `POST` | `/api/v1/admin/log-level` | 관리자(`settings:write`) | 런타임 로그 수준 변경(`fatal`, `error`, `warn`, `info`, `debug`, `trace`, `silent`) |
| `GET` | `/api/v1/metrics` | 관리자(`system:health`) | 텍스트 형식의 Prometheus 메트릭 |
| `GET` | `/api/v1/admin/support-bundle` | 관리자(`system:health`) | 편집 삭제된 진단 지원 번들 ZIP 다운로드 |
| `GET` | `/api/v1/admin/usage` | 관리자(`audit:read`) | 선택적 `days` 쿼리 파라미터가 있는 사용량 대시보드 데이터 |
...
이 라우트들은 관련 엔터프라이즈 기능으로 라이선스 게이트가 걸려 있습니다. 그럼에도 나열된 SnapOtter 권한이 필요합니다.

| 메서드 | 경로 | 접근 권한 | 설명 |
|--------|------|--------|-------------|
| `GET` | `/api/v1/enterprise/audit/export` | 관리자(`audit:read`) | 필터와 함께 감사 항목을 JSON 또는 CSV로 내보내기 |
| `GET` | `/api/v1/enterprise/config/export` | 관리자(`system:health`) | 편집 삭제된 인스턴스 구성, 사용자 지정 역할, 팀 내보내기 |
...
| `GET` | `/api/v1/enterprise/legal-hold` | 관리자(`compliance:manage`) | 사용자 및 팀 법적 보존 목록 조회 |
| `PUT` | `/api/v1/enterprise/legal-hold` | 관리자(`compliance:manage`) | 사용자 또는 팀에 법적 보존 적용 또는 해제 |
| `POST` | `/api/v1/enterprise/scim/token` | 관리자(`users:manage`) | SCIM 베어러 토큰 생성, 한 번만 반환 |
...
| `GET` | `/api/v1/enterprise/users/:id/export/:jobId` | 관리자(`compliance:manage`) | GDPR 내보내기 상태와 다운로드 URL 읽기 |
...
| `GET` | `/api/v1/admin/upgrade-check` | 관리자(`system:health`) | 업그레이드 준비 상태 점검 실행 |

### SCIM 2.0 {#scim-2-0}

SCIM 검색 엔드포인트는 공개입니다. 사용자 및 그룹 엔드포인트는 위에서 생성한 SCIM 베어러 토큰이 필요합니다.

| 메서드 | 경로 | 접근 권한 | 설명 |
|--------|------|--------|-------------|
| `GET` | `/api/v1/scim/v2/ServiceProviderConfig` | 공개 | SCIM 서버 기능 |
| `GET` | `/api/v1/scim/v2/Schemas` | 공개 | SCIM 스키마 검색 |
| `GET` | `/api/v1/scim/v2/ResourceTypes` | 공개 | SCIM 리소스 유형 검색 |
| `GET` | `/api/v1/scim/v2/Users` | SCIM 토큰 | 선택적 SCIM 필터와 함께 사용자 목록 |
| `POST` | `/api/v1/scim/v2/Users` | SCIM 토큰 | 사용자 생성 |
| `GET` | `/api/v1/scim/v2/Users/:id` | SCIM 토큰 | 사용자 조회 |
| `PUT` | `/api/v1/scim/v2/Users/:id` | SCIM 토큰 | 사용자 교체 |
| `DELETE` | `/api/v1/scim/v2/Users/:id` | SCIM 토큰 | 사용자 소프트 비활성화 |
| `GET` | `/api/v1/scim/v2/Groups` | SCIM 토큰 | 팀을 SCIM 그룹으로 목록 조회 |
| `POST` | `/api/v1/scim/v2/Groups` | SCIM 토큰 | 팀 생성 |
| `GET` | `/api/v1/scim/v2/Groups/:id` | SCIM 토큰 | 팀 조회 |
...
| 메서드 | 경로 | 접근 권한 | 설명 |
|--------|------|--------|-------------|
| `GET` | `/api/v1/meme-templates` | 인증 필요 | 텍스트 상자 위치와 함께 사용 가능한 모든 밈 템플릿 목록 조회 |
| `GET` | `/api/v1/meme-templates/full/:filename` | 인증 필요 | 전체 크기 템플릿 이미지 제공 |
| `GET` | `/api/v1/meme-templates/thumbs/:filename` | 인증 필요 | 템플릿 썸네일 제공 |
| `GET` | `/api/v1/meme-templates/fonts/:filename` | 인증 필요 | 밈 텍스트 렌더링에 사용되는 폰트 파일 제공 |
...
| 403 | 권한 부족 |
| 404 | 리소스를 찾을 수 없음 |
| 413 | 파일이 너무 큼(`MAX_UPLOAD_SIZE_MB` 참조) |
| 422 | 유효성 검사 후 처리 실패 |
| 429 | 속도 제한(`RATE_LIMIT_PER_MIN` 참조) |
| 501 | 필요한 AI 기능 번들이 설치되지 않음(`FEATURE_NOT_INSTALLED`) |
| 500 | 내부 서버 오류 |
