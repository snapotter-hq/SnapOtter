...
**المعالجة:** إرجاع `200` مع JSON عند انتهاء OCR داخل النافذة المتزامنة. تعود المهام الأطول `202`؛ اتبع تدفق تقدم SSE الخاص بالمهمة إلى الحدث النهائي الخاص بها، والذي يحتوي `result` الخاص به على نفس حقول OCR.
...
إذا تم توفير حقل نموذج `clientJobId`، فسيتم دفق أحداث التقدم. تعني استجابة `202` أن العميل يجب أن يبقي الدفق مفتوحًا حتى حدث `complete` أو `failed` في المحطة الطرفية:
...
- تبلغ الاستجابة عن كل من `requestedQuality` و`actualQuality`، بالإضافة إلى المحرك والجهاز والموفر ووقت التشغيل وإصدارات الطراز وأي تحذيرات.
