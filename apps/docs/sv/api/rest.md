---
description: "Komplett referens för REST API. Verktygsslutpunkter, batchbearbetning, pipelines, filbibliotek, autentisering, team och administrationsåtgärder."
i18n_source_hash: eb73a14533a1
i18n_provenance: human
i18n_output_hash: a12f03f90dc4
---

# REST API-referens {#rest-api-reference}

Interaktiv API-dokumentation med exempel på begäran och svar finns på [http://localhost:1349/api/docs](http://localhost:1349/api/docs).

Maskinläsbara specifikationer:
- `/api/v1/openapi.yaml` - OpenAPI 3.1-specifikation
- `/llms.txt` - LLM-vänlig sammanfattning
- `/llms-full.txt` - Komplett LLM-vänlig dokumentation
...
Sessioner går ut efter 7 dagar (konfigurerbart via `SESSION_DURATION_HOURS`).
...
| `POST` | `/api/auth/mfa/disable` | Auth | Inaktivera MFA för den aktuella användaren |
...
| `GET` | `/api/auth/oidc/callback` | Publik | OIDC-auktoriseringsåterkoppling |
...
När MFA är aktiverat för en användare returnerar `POST /api/auth/login` `{"requiresMfa":true,"mfaToken":"...","mfaRequired":true|false}` i stället för en sessionstoken. Skicka den `mfaToken` plus en TOTP- eller återställningskod till `/api/auth/mfa/complete`.
...
| `GET` | `/api/v1/readyz` | Publik | Readiness-prob. Kontrollerar PostgreSQL, Redis, diskutrymme och S3 när det är konfigurerat. Returnerar 503 när instansen inte bör ta emot trafik. |
...
- `clientJobId` är ett valfritt formulärfält för anroparbaserad förloppskorrelation.
- `fileId` är ett valfritt formulärfält som refererar till ett befintligt filbiblioteksobjekt. När det finns med sparas den bearbetade utdatan som en ny version och svaret innehåller `savedFileId`.
...
- **Alla köade verktyg** kan returnera 202 JSON om de är långkörande eller överskrider det synkrona väntefönstret: `{"jobId":"...","async":true}`. Anslut till SSE för förlopp och ladda sedan ned när det är klart (se [Förloppsspårning](#progress-tracking)).
- **Batch**-rutter returnerar ett ZIP-arkiv som strömmas direkt (med `X-Job-Id`-header) för verktyg som är registrerade i det generiska batch-registret.

## Verktygsreferens {#tools-reference}

### Konverteringsförinställningar {#conversion-presets}

Den delade katalogen inkluderar 83 dedikerade slutpunkter för konverteringsförinställningar, till exempel `jpg-to-png`, `mov-to-mp4`, `m4a-to-mp3`, `pdf-to-jpg` och `excel-to-csv`. Förinställningar är förstaklassiga verktygsrutter:

`POST /api/v1/tools/<section>/<presetId>`

Varje förinställning låser utdataformatet och delegerar till ett basverktyg som `convert`, `convert-video`, `extract-audio`, `convert-audio`, `image-to-pdf`, `pdf-to-image`, `svg-to-raster` eller `convert-spreadsheet`. Se [Konverteringsförinställningar](/sv/tools/conversion-presets) för den kompletta rutttabellen och valfria inställningar.
...
| `bulk-rename` | Massbyt namn | `pattern` (stöder `{n}`, `{date}`, `{original}`), `startIndex`, `padding` |
...
| `sharpening` | Skärpning | `method` (adaptive/unsharp-mask/high-pass), `sigma`, `m1`, `m2`, `x1`, `y2`, `y3`, `amount`, `radius`, `threshold`, `strength`, `kernelSize` (3/5), `denoise` (off/light/medium/strong) |
...
Alla AI-verktyg körs på din hårdvara: CPU som standard, eller NVIDIA CUDA när en NVIDIA-GPU som stöds är tillgänglig. Acceleration via Intel/AMD iGPU genom VA-API, Quick Sync eller OpenCL stöds i dagsläget inte för AI-inferens. Ingen internetanslutning krävs.
...
| `erase-object` | Objektradering | LaMa (ONNX) | Mask skickas som andra fildel (fältnamn `mask`), `format`, `quality` |
| `ocr` | OCR / textutdrag | PaddleOCR / Tesseract | `quality` (fast/balanced/best), `language`, `enhance` |
...
| `noise-removal` | Brusborttagning | Nivåindelad brusreducering | `tier` (quick/balanced/quality/maximum), `strength`, `detailPreservation`, `colorNoise`, `format`, `quality` |
| `red-eye-removal` | Borttagning av röda ögon | Ansiktslandmärken + färganalys | `sensitivity`, `strength` |
...
| `passport-photo` | Passfoto | MediaPipe-landmärken | Tvåfasflöde. Analysen använder multipart `file`; genereringen använder JSON med `countryCode`, `bgColor`, `printLayout` (none/4x6/a4), landmärken, bildmått |
...
| `background-replace` | Byt bakgrund | rembg (BiRefNet) | `backgroundType` (color/gradient), `color` (hex), `gradientColor1`, `gradientColor2`, `gradientAngle`, `feather` (0-20), `format` (png/webp) |
| `blur-background` | Gör bakgrund suddig | rembg (BiRefNet) | `intensity` (1-100), `feather` (0-20), `format` (png/webp) |
| `ai-canvas-expand` | AI-canvasexpansion | LaMa (outpainting) | `extendTop`, `extendRight`, `extendBottom`, `extendLeft` (px), `tier` (fast/balanced/high), `format`, `quality` |
...
| `compose` | Bildkomposition | `x`, `y`, `opacity`, `blend` - andra filen läggs i lager ovanpå |
| `meme-generator` | Meme-generator | `templateId`, `textLayout` (top-bottom/top-only/bottom-only/center/side-by-side), `textBoxes` ([{id, text}]), `fontFamily` (anton/arial-black/comic-sans/montserrat/bebas-neue/permanent-marker/roboto), `fontSize`, `textColor`, `strokeColor`, `textAlign`, `allCaps`. Stöder mallläge (JSON-brödtext med `templateId`) eller anpassat bildläge (multipart med fil). |

### Verktyg {#utilities}

| Verktygs-ID | Namn | Nyckelinställningar |
|---------|------|-------------|
| `info` | Bildinfo | - (returnerar width, height, format, size, channels, hasAlpha, DPI, EXIF) |
| `compare` | Bildjämförelse | `mode` (side-by-side/overlay/diff), `diffThreshold` - andra filen är jämförelsemålet |
| `find-duplicates` | Hitta dubbletter | `threshold` (perceptuellt hash-avstånd, standard 8) - flera filer |
| `color-palette` | Färgpalett | `count` (antal dominerande färger), `format` (hex/rgb) |
...
| `barcode-generate` | Streckkodsgenerator | `text`, `type` (code128/ean13/upca/code39/itf14/datamatrix), `scale` (1-8), `includeText` (bool). JSON-brödtext, ingen filuppladdning. |

### Layout och komposition {#layout-composition}

| Verktygs-ID | Namn | Nyckelinställningar |
|---------|------|-------------|
| `collage` | Collage / rutnät | `template` (25+ layouter), `gap`, `backgroundColor`, `borderRadius` - flera filer |
| `stitch` | Sammanfoga / kombinera | `direction` (horizontal/vertical/grid), `gap`, `backgroundColor`, `alignment` - flera filer |
| `split` | Bilddelning | `mode` (grid/rows/cols), `rows`, `cols`, `tileWidth`, `tileHeight` |
| `border` | Kant och ram | `width`, `color`, `style` (solid/gradient/pattern), `borderRadius`, `padding`, `shadow` |
...
| `sprite-sheet` | Sprite-ark | `columns` (1-16), `padding`, `background` (hex), `format` (png/webp/jpeg), `quality` - flera filer (2-64 bilder) |
...
| `trim-video` | Klipp video | `startS`, `endS`, `precise` (bool, bildruteexakt klippning) |
...
| `reverse-video` | Spela upp video baklänges | - (max 5 minuter) |
| `video-loudnorm` | Normalisera ljud | - (EBU R128) |
| `aspect-pad` | Utfyllnad efter bildformat | `target` (16:9/9:16/1:1/4:3/3:4), `color` (hex) |
| `blur-pad` | Suddig utfyllnad | `target` (16:9/9:16/1:1/4:3/3:4), `blur` (2-50) |
| `watermark-video` | Vattenstämpel på video | `text`, `position`, `fontSize`, `opacity`, `color` |
...
| `merge-videos` | Sammanfoga videor | - (flera filer, normaliserade till första videons upplösning) |
...
| `extract-subtitles` | Extrahera undertexter | - (matar ut SRT) |
| `images-to-video` | Bilder till video | `secondsPerImage` (0.5-10), `resolution` (1080p/720p/square), `fps` - flera filer |
...
| `trim-audio` | Klipp ljud | `startS`, `endS` |
...
| `reverse-audio` | Spela upp ljud baklänges | - |
| `audio-speed` | Ljudhastighet | `factor` (0.25-4) |
| `pitch-shift` | Tonhöjdsförskjutning | `semitones` (-12 till 12) |
| `audio-channels` | Ljudkanaler | `mode` (stereo-to-mono/mono-to-stereo/swap) |
| `silence-removal` | Tystnadsborttagning | `thresholdDb` (-80 till -20), `minSilenceS` (0.1-5) |
| `noise-reduction` | Brusreducering | `strength` (light/medium/strong) |
| `merge-audio` | Sammanfoga ljud | `format` (mp3/wav/flac/m4a) - flera filer |
...
| Verktygs-ID | Namn | Nyckelinställningar |
|---------|------|-------------|
| `merge-pdf` | Sammanfoga PDF-filer | - (flera filer, upp till 20 PDF-filer) |
...
| `organize-pdf` | Organisera PDF | `order` (qpdf-sidordning, t.ex. \"3,1,2,5-z\") |
...
| `grayscale-pdf` | Gråskale-PDF | - |
| `pdfa-convert` | PDF/A-konvertering | - (arkiv-PDF/A-2) |
...
| `watermark-pdf` | Vattenstämpel på PDF | `text`, `position`, `fontSize`, `opacity`, `rotation` |
| `pdf-page-numbers` | Sidnummer i PDF | `position` (bl/bc/br/tl/tc/tr), `fontSize` |
| `flatten-pdf` | Platta ut PDF | - (bakar in formulär och anteckningar) |
| `redact-pdf` | Redigera bort i PDF | `terms` (string[]), `caseSensitive` (bool) |
| `sign-pdf` | Signera PDF | Anpassad multipart-rutt med PDF `file`, signaturfiler `sig0`, `sig1` och JSON-array `placements` |
...
| `convert-spreadsheet` | Konvertera kalkylblad | `format` (xlsx/ods/csv) |
...
| `merge-csvs` | Sammanfoga CSV-filer | - (flera filer, matchande kolumner) |
...
| `create-zip` | Skapa ZIP | - (flera filer, 2-50 filer) |
| `extract-zip` | Extrahera ZIP | - (bombskyddad) |

### HTML till bild {#html-to-image}

Fånga en webbsida som en bild. Till skillnad från andra verktyg accepterar den här slutpunkten `application/json` i stället för multipart-formulärdata (ingen filuppladdning behövs).
...
|-----------|------|---------|-------------|
| `url` | string | (krävs) | URL att fånga (endast http/https) |
...
| `viewportWidth` | number | `1280` | Anpassad visningsvidd 320-3840 |
| `viewportHeight` | number | `720` | Anpassad visningshöjd 320-2160 |
...
### Underrutter för verktyg {#tool-sub-routes}

Vissa verktyg exponerar ytterligare slutpunkter utöver standarden `POST /api/v1/tools/<section>/<toolId>`:

| Metod | Sökväg | Beskrivning |
|--------|------|-------------|
| `GET` | `/api/v1/tools/popular` | Returnera populära verktygs-ID, med reservlösning till en kurerad standardlista när användningsdata är gles |
...
| `POST` | `/api/v1/tools/image/passport-photo/analyze` | Fas 1: AI-ansiktsigenkänning + bakgrundsborttagning. Returnerar ansiktslandmärken och cachad data. |
...
| `POST` | `/api/v1/tools/image/gif-tools/info` | Hämta GIF-metadata (antal bildrutor, mått, längd) |
| `POST` | `/api/v1/tools/pdf/pdf-to-image/info` | Hämta PDF-metadata (antal sidor, mått) |
| `POST` | `/api/v1/tools/pdf/pdf-to-image/preview` | Generera en förhandsgranskning av en specifik PDF-sida |
| `POST` | `/api/v1/tools/pdf/pdf-to-jpg/info` | Hämta PDF-metadata för den dedikerade JPG-förinställningen |
| `POST` | `/api/v1/tools/pdf/pdf-to-jpg/preview` | Generera en förhandsgranskning av en PDF-sida med JPG-förinställningen |
| `POST` | `/api/v1/tools/pdf/pdf-to-png/info` | Hämta PDF-metadata för den dedikerade PNG-förinställningen |
| `POST` | `/api/v1/tools/pdf/pdf-to-png/preview` | Generera en förhandsgranskning av en PDF-sida med PNG-förinställningen |
| `POST` | `/api/v1/tools/pdf/pdf-to-tiff/info` | Hämta PDF-metadata för den dedikerade TIFF-förinställningen |
| `POST` | `/api/v1/tools/pdf/pdf-to-tiff/preview` | Generera en förhandsgranskning av en PDF-sida med TIFF-förinställningen |
| `POST` | `/api/v1/tools/image/svg-to-raster/batch` | Batchkonvertera flera SVG:er till raster |
| `POST` | `/api/v1/tools/image/image-enhancement/analyze` | Analysera bildkvalitet och returnera rekommendationer för förbättring |
| `POST` | `/api/v1/tools/image/optimize-for-web/preview` | Lättviktig förhandsgranskning för justering av parametrar i realtid. Returnerar optimerad bild med storlekshuvuden. |

## Batchbearbetning {#batch-processing}

Applicera ett generiskt batch-aktiverat verktyg på flera filer samtidigt. Returnerar ett ZIP-arkiv. Anpassade rutter för flera filer eller flera steg, till exempel PDF-signering, PDF OCR och PDF-till-bild-förinställningsrutter, använder sitt eget slutpunktskontrakt i stället för den generiska `/batch`-rutten.
...
Samtidigheten styrs av `CONCURRENT_JOBS` (standard: autodetekterad från CPU-kärnor). `MAX_BATCH_SIZE` begränsar antalet filer per batch (standard: 100; sätt 0 för obegränsat).
...
| `GET` | `/api/v1/pipeline/list` | Lista sparade pipelines (administratörer ser alla; användare ser sina egna) |
| `DELETE` | `/api/v1/pipeline/:id` | Ta bort (ägare eller admin) |
| `GET` | `/api/v1/pipeline/tools` | Lista verktygs-ID som är giltiga för pipelinesteg |

## Förloppsspårning {#progress-tracking}

Långkörande jobb, köade verktyg, batchjobb och pipelines skickar förlopp i realtid via Server-Sent Events. Förloppsströmmen är publik och nycklad efter jobb-ID, så klienter behöver inte skicka en Authorization-header för att läsa den.
...
Du kan begära att ett köat eller körande jobb avbryts med `POST /api/v1/jobs/:jobId/cancel`. Svaret är `{"canceled":true|false}`.

## Filbibliotek {#file-library}

Beständig fillagring med versionshistorik.

| Metod | Sökväg | Beskrivning |
|--------|------|-------------|
| `POST` | `/api/v1/upload` | Ladda upp filer till arbetsytan (tillfällig bearbetning) |
| `POST` | `/api/v1/files/upload` | Ladda upp filer till det beständiga filbiblioteket |
| `POST` | `/api/v1/files/save-result` | Spara ett verktygsbearbetningsresultat som en ny filversion |
| `GET` | `/api/v1/files` | Lista sparade filer (paginerat, med sökning) |
| `GET` | `/api/v1/files/:id` | Hämta filmetadata + versionskedja |
| `GET` | `/api/v1/files/:id/download` | Ladda ned fil |
| `GET` | `/api/v1/files/:id/thumbnail` | Hämta 300px JPEG-miniatyr |
| `DELETE` | `/api/v1/files` | Massradera filer och deras versionskedjor (brödtext: `{ ids: [...] }`) |
| `POST` | `/api/v1/fetch-urls` | Hämta fjärr-URL:er till arbetsytan för URL-baserade importer |
| `POST` | `/api/v1/preview` | Generera en webbläsarkompatibel WebP-förhandsgranskning (för HEIC/HEIF/RAW-format) |
| `GET` | `/api/v1/files/:id/preview` | Strömma en cachad eller genererad webbläsarkompatibel förhandsgranskning för en sparad PDF, ett Office-dokument, en video eller en ljudfil |
| `POST` | `/api/v1/preview/generate` | Generera en on-demand MP4- eller MP3-förhandsgranskning för en uppladdad mediefil utan att spara den först |
| `GET` | `/api/v1/download/:jobId/:filename` | Ladda ned en bearbetad fil från en arbetsyta |

För att automatiskt spara ett verktygsresultat i biblioteket, inkludera `fileId` som ett multipart-formulärfält som refererar till en befintlig biblioteksfil. Det bearbetade resultatet sparas som en ny version.
...
Nyckel-värde-konfiguration vid körning (läses av alla autentiserade användare, skrivs endast av admin).

| Metod | Sökväg | Beskrivning |
|--------|------|-------------|
| `GET` | `/api/v1/settings` | Hämta alla inställningar |
| `PUT` | `/api/v1/settings` | Massuppdatera inställningar (JSON-brödtext med nyckel-värde-par) |
| `GET` | `/api/v1/settings/:key` | Hämta en specifik inställning efter nyckel |

Kända nycklar: `disabledTools` (JSON-array med verktygs-ID), `enableExperimentalTools` (bool-sträng), `loginAttemptLimit` (nummer).

## Preferenser {#preferences}

Preferenser per användare är åtskilda från instansinställningar. Alla autentiserade användare kan läsa och uppdatera sin egen preferenskarta.

| Metod | Sökväg | Beskrivning |
|--------|------|-------------|
| `GET` | `/api/v1/preferences` | Hämta den aktuella användarens preferenser som `{ "preferences": { ... } }` |
| `PUT` | `/api/v1/preferences` | Skapa eller uppdatera en eller flera preferensnycklar för den aktuella användaren |
...
| Metod | Sökväg | Åtkomst | Beskrivning |
|--------|------|--------|-------------|
| `GET` | `/api/v1/roles` | Admin (`audit:read`) | Lista alla roller med användarantal |
...
| `DELETE` | `/api/v1/roles/:id` | Admin (`security:manage`) | Ta bort en anpassad roll (kan inte ta bort inbyggda roller; berörda användare återgår till rollen `user`) |
...
Slutpunkt endast för admin för att granska säkerhetsrelevanta åtgärder.

| Metod | Sökväg | Åtkomst | Beskrivning |
|--------|------|--------|-------------|
| `GET` | `/api/v1/audit-log` | Admin (`audit:read`) | Paginerad granskningslogg med valfria filter |
...
| Metod | Sökväg | Åtkomst | Beskrivning |
|--------|------|--------|-------------|
| `GET` | `/api/v1/config/analytics` | Publik | Hämta den effektiva analyskonfigurationen (PostHog-nyckel, Sentry DSN, samplingsfrekvens). Nycklar, DSN och instans-ID är tomma när analys är avstängd, antingen från inbakningen vid kompilering eller instansinställningen `analyticsEnabled`. |
| `POST` | `/api/v1/feedback` | Auth | Skicka explicit användarfeedback till det konfigurerade PostHog-projektet som `feedback_submitted`. Rutten respekterar analysgrinden, hastighetsbegränsar inskickningar, tar bort kontaktfält om inte `contactOk` är true och accepterar aldrig filinnehåll, filnamn, uppladdningssökvägar eller rå privat feltext. När analys är inaktiverat returnerar den `{ "ok": true, "accepted": false }`. |
| `PUT` | `/api/v1/settings` | Admin (`settings:write`) | Ställ in det instansomfattande avanmälningsvalet. Skicka en JSON-brödtext `{ "analyticsEnabled": "false" }` för att stänga av analys för alla, eller `"true"` för att slå på den igen. |

## Funktioner / AI-paket {#features-ai-bundles}

Hantera AI-funktionspaket (installera/avinstallera AI-modellpaket i Docker-miljön).

| Metod | Sökväg | Åtkomst | Beskrivning |
|--------|------|--------|-------------|
| `GET` | `/api/v1/features` | Auth | Lista alla funktionspaket och deras installationsstatus |
| `POST` | `/api/v1/admin/features/:bundleId/install` | Admin (`features:manage`) | Installera ett funktionspaket (asynkront, returnerar `jobId` för förloppsspårning) |
| `POST` | `/api/v1/admin/features/:bundleId/uninstall` | Admin (`features:manage`) | Avinstallera ett funktionspaket och rensa upp modellfiler |
| `GET` | `/api/v1/admin/features/disk-usage` | Admin (`features:manage`) | Hämta total diskanvändning för AI-modeller |
| `POST` | `/api/v1/admin/features/import` | Admin (`features:manage`) | Importera ett offline AI-paketarkiv |

## Administrationsåtgärder {#admin-operations}

Driftslutpunkter för observerbarhet, support, användningsrapportering och backupstatus.

| Metod | Sökväg | Åtkomst | Beskrivning |
|--------|------|--------|-------------|
| `GET` | `/api/v1/admin/log-level` | Admin (`settings:write`) | Läs den aktuella loggnivån vid körning |
| `POST` | `/api/v1/admin/log-level` | Admin (`settings:write`) | Ändra loggnivån vid körning (`fatal`, `error`, `warn`, `info`, `debug`, `trace` eller `silent`) |
| `GET` | `/api/v1/metrics` | Admin (`system:health`) | Prometheus-mätvärden i textformat |
| `GET` | `/api/v1/admin/support-bundle` | Admin (`system:health`) | Ladda ned ett redigerat diagnostiskt supportpaket som ZIP |
| `GET` | `/api/v1/admin/usage` | Admin (`audit:read`) | Data för användningspanelen, med valfri frågeparameter `days` |
| `GET` | `/api/v1/admin/backup-status` | Admin (`system:health`) | Läs den senaste backup-metadatan och färskhetsstatusen |
...
| `GET` | `/api/v1/enterprise/ip-allowlist` | Admin (`security:manage`) | Läs konfigurerad CIDR-tillåtelselista |
| `PUT` | `/api/v1/enterprise/ip-allowlist` | Admin (`security:manage`) | Uppdatera CIDR-tillåtelselista med skydd mot att låsa ut sig själv |
| `GET` | `/api/v1/enterprise/legal-hold` | Admin (`compliance:manage`) | Lista rättsliga spärrar för användare och team |
| `PUT` | `/api/v1/enterprise/legal-hold` | Admin (`compliance:manage`) | Tillämpa eller häv en rättslig spärr på en användare eller ett team |
...
| `GET` | `/api/v1/enterprise/siem/config` | Admin (`webhooks:manage`) | Läs konfiguration för SIEM-vidarebefordran |
| `PUT` | `/api/v1/enterprise/siem/config` | Admin (`webhooks:manage`) | Uppdatera konfiguration för SIEM-vidarebefordran |
...
| `POST` | `/api/v1/enterprise/webhooks/:index/test` | Admin (`webhooks:manage`) | Skicka en test-webhook-payload |
...
SCIM-upptäcktsslutpunkter är publika. Slutpunkter för användare och grupper kräver SCIM-bärartoken som genererats ovan.
...
| `DELETE` | `/api/v1/scim/v2/Users/:id` | SCIM-token | Inaktivera en användare mjukt |
...
## Meme-mallar {#meme-templates}

Stödjande API för meme-generatorverktyget.

| Metod | Sökväg | Åtkomst | Beskrivning |
|--------|------|--------|-------------|
| `GET` | `/api/v1/meme-templates` | Auth | Lista alla tillgängliga meme-mallar med textboxpositioner |
...
| `GET` | `/api/v1/meme-templates/fonts/:filename` | Auth | Servera typsnittsfilen som används för rendering av meme-text |
...
| Status | Betydelse |
|--------|---------|
| 400 | Ogiltig begäran / validering misslyckades |
| 401 | Inte autentiserad |
| 403 | Otillräckliga behörigheter |
| 404 | Resursen hittades inte |
| 413 | Filen är för stor (se `MAX_UPLOAD_SIZE_MB`) |
| 422 | Bearbetningen misslyckades efter validering |
| 429 | Hastighetsbegränsad (se `RATE_LIMIT_PER_MIN`) |
| 501 | Nödvändigt AI-funktionspaket är inte installerat (`FEATURE_NOT_INSTALLED`) |
