...
## スマートフォンから使う {#use-it-from-your-phone}

SnapOtter はモバイルブラウザで動作し、アプリとしてインストールすることもできます。スマートフォンでインスタンスを開き、次のように操作します。

- **iPhone / iPad（Safari）**: 共有をタップし、**ホーム画面に追加** をタップします。
- **Android（Chrome）**: ブラウザのメニューを開き、**アプリをインストール** をタップします。

インストールしたアプリは専用のウィンドウで開き、そのままインスタンスにつながります。

注意点がひとつあります。ブラウザがインストールを提案するのは HTTPS 経由のときだけです。LAN 上の通常の HTTP アドレスでもブラウザのタブでは問題なく使えますが、実際にインストールするには、証明書を備えたリバースプロキシの背後にインスタンスを配置してください（[デプロイガイド](/ja/guide/deployment) を参照）。

スマートフォンやタブレットでは、画像ツールのアップロードボタンの横に **写真を撮る** ボタンが表示されます。レシートやホワイトボードを撮影すると、そのままツールに取り込まれます。
