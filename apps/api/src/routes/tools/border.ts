...
const settingsSchema = z.object({
  borderWidth: z.number().int().min(0).max(2000).default(10),
  borderColor: hexColor.default("#000000"),
  padding: z.number().int().min(0).max(200).default(0),
  paddingColor: hexColor.default("#FFFFFF"),
  cornerRadius: z.number().int().min(0).max(2000).default(0),
  shadow: z.boolean().default(false),
  shadowBlur: z.number().int().min(1).max(200).default(15),
  shadowOffsetX: z.number().int().min(-50).max(50).default(0),
  shadowOffsetY: z.number().int().min(-50).max(50).default(5),
