...
- The public sites (snapotter.com, docs.snapotter.com, demo.snapotter.com) hold the same line, and the risk there sits outside the repo: Cloudflare zone features can inject scripts at the edge, and Web Analytics did exactly that (#793). Zone-level Web Analytics stays disabled in the Cloudflare dashboard, and every Pages deploy workflow ends by fetching the live page as a browser would and failing on any origin that isn't the page's own or allowlisted (`scripts/check-live-egress.mjs`).
