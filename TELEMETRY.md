...
- The public sites (snapotter.com, docs.snapotter.com, demo.snapotter.com) are our own marketing and docs surfaces, not a user's instance, and they run two destinations: PostHog through the e.snapotter.com proxy, and Cloudflare Web Analytics (cookieless, no fingerprinting, injected at the zone edge; decided on #793). Nothing else. The risk sits outside the repo, because Cloudflare zone features can add scripts at the edge without a commit, so every Pages deploy workflow ends by fetching the live page as a browser would and failing on any origin that isn't the page's own or allowlisted (`scripts/check-live-egress.mjs`).
