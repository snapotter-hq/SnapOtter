...
for (const loc of LOCALES) {
  for (const entryPoint of ENTRY_POINTS) {
    test(`${loc}${entryPoint}: English-only tool-detail & self-hosted links are not locale-prefixed`, async ({
      page,
    }) => {
      ...
      expect(
        badToolDetail,
        `localized tool-detail links on ${pagePath} (must be un-prefixed /tools/...): ${badToolDetail.slice(0, 3).join(", ")}`,
      ).toEqual([]);
      ...
      expect(
        badSelfHosted,
        `localized self-hosted links on ${pagePath}: ${badSelfHosted.join(", ")}`,
      ).toEqual([]);
